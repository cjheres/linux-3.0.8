...
#include <global.h> /* MEMMAP_CHIP_CTRL_REG_BASE, RA_Gbl_pinMux, RA_Gbl_pinMux1 */
#include <apb_perf_base.h> /* SM_SYS_CTRL_REG_BASE */
#include <SysMgr.h> /* RA_smSysCtl_SM_GSM_SEL */
...
#define NAND_CACHE_INTER_BUF	1
#define NUM_INTER_BUF		4
#define INVALID_PAGE		0xfffffff
#define NAND_MAX_IOSIZE	(NAND_MAX_PAGESIZE + NAND_MAX_OOBSIZE)
#define NAND_CACHE_FLUSH_TIMEOUT	20
...
struct pxa3xx_nand {
	...
	/* cache program related */
	unsigned char		*last_wr_buf[NUM_CHIP_SELECT][NUM_INTER_BUF];
	unsigned char		*last_wr_oob_buf[NUM_CHIP_SELECT][NUM_INTER_BUF];
	uint32_t		wr_idx[NUM_CHIP_SELECT];
	uint32_t		buf_depth[NUM_CHIP_SELECT];
	uint32_t		wr_page[NUM_CHIP_SELECT][NUM_INTER_BUF];
	uint32_t		write_size[NUM_CHIP_SELECT];
	uint32_t		write_oob_size[NUM_CHIP_SELECT];
	ktime_t		last_wrtime[NUM_CHIP_SELECT];
	uint32_t		reg_ndsr[NUM_CHIP_SELECT];
	bool			new_progam_cmd[NUM_CHIP_SELECT];
	bool			cache_program_en;
	bool			skip_following_status_read[NUM_CHIP_SELECT];
	bool			flush_needed;
	...
	int			dp_write_cache_desc_len_p1[NUM_CHIP_SELECT];
	int			dp_write_cache_desc_len_p2[NUM_CHIP_SELECT];
	dma_addr_t 		dp_write_desc_cache_addr1;
	unsigned char		*dp_write_cache_desc1;

	dma_addr_t 		dp_write_desc_cache_addr2;
	unsigned char		*dp_write_cache_desc2;
...
static int use_cache_program = 1;
module_param(use_cache_program, bool, 0444);
MODULE_PARM_DESC(use_cache_program, "enable NAND cache nand dp programe");
...
static struct pxa3xx_nand_cmdset default_cmdset = {
	...
	.dual_plane_wr_cache = 0x15811180,
};
...
static void debug_info_print(struct pxa3xx_nand *nand)
{
	...
	for (i = 0; i < TEST_NUM; i++) {
		...
		printk("Num[%02d] CMD[%04x] page_addr[%08x] planes[%d] buff[%08x] "
			"NDSR[%08x] ent[%08x] ext[%08x]\n",
			num, nand_debug_info[num].cmd, nand_debug_info[num].page_addr,
			nand_debug_info[num].planes, nand_debug_info[num].buff,
			nand_debug_info[num].status, time_ent, time_ext);
	...
	data_cp_sem = pb_readl(nand, RA_pBridge_dHub + RA_dHubReg2D_dHub
	...
	printk("DATA_CP %08x\n", data_cp_sem);
	{
		int m, data;
		printk("\n ----------------------------------------------------\n");
		printk("RA_pBridge_dHub RA_dHubReg2D_dHub dump %08x\n", RA_pBridge_dHub);
		for (m = 0; m < 0x200; m += 4) {
			if ((m & 0xf) == 0) {
				printk("\n 0x%04x   ", m);
			}
			data = pb_readl(nand, RA_pBridge_dHub + m);
			printk("%08x ", data);
		}
		printk("\n ----------------------------------------------------\n");
	}
...
static void debug_push_staus(unsigned int status)
{
	nand_debug_info[current_num].status	= status;
	nand_debug_info[current_num].finish_time= ktime_get();
	nand_debug_info[current_num].step	= 1;
}
/************************************************************/
#endif
...
static void pxa3xx_nand_dump_registers(struct pxa3xx_nand_info *info)
{
	...
	printk(KERN_INFO "pinmux_soc  = %08X\n",
			__raw_readl(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_pinMux));
	printk(KERN_INFO "pinmux_soc1 = %08X\n",
			__raw_readl(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_pinMux1));
	printk(KERN_INFO "pinmux_sm   = %08X\n",
			__raw_readl(SM_SYS_CTRL_REG_BASE + RA_smSysCtl_SM_GSM_SEL));
	printk(KERN_INFO "================================\n");
}
...
static irqreturn_t mv88dexx_nand_dma_intr(int irq, void *devid)
{
	...
	nand->reg_ndsr[nand->chip_select] = status;
#if (PXA3XX_NAND_DMA_DEBUG == 1)
	nand->status_rec |= status;
#endif
...
static inline int is_read_datbuf_blank(uint8_t *buf, size_t len)
{
	int i = 0;
	uint8_t *b = buf;
	...
	for (; len > 0; len--, i++, buf++) {
		if (*buf != 0xff) {
			if (i == 0x7C8 || i == 0xFC8 || i == 0xFCD || i == 0x17C8 || i == 0x1FCD
					|| i == 0x27C8 || i == 0x2FC8 || i == 0x37C8 || i == 0x3FCD) {
				*buf = 0xff;
				continue;
			}
		...
		uint16_t column, int page_addr)
{
	...
	case NAND_CMD_NAND_CACHE_FLUSH:
	...
	case NAND_CMD_NAND_CACHE_FLUSH:
		exec_cmd = 0;
		if (likely(use_dma)) {
			if ((info->n_planes == 2) && nand->buf_depth[nand->chip_select]
				&& nand->cache_program_en) {
				nand->ecc_strength = info->ecc_strength;
				nand->total_cmds = chunks + 1;
				exec_cmd = 1;
			}
...
#if (PXA3XX_NAND_DMA_DEBUG == 1)
static inline int wait_dhub_ready(struct pxa3xx_nand *nand)
{
	uint32_t read;
	int loop = 0;
...
#if (PXA3XX_NAND_DMA_DEBUG == 1)
	if (wait_dhub_ready(nand) == -1) { goto retry;}
#else
	wait_dhub_ready(nand);
#endif
}
...
/**
 * mv88dexx_nand_dma_fill_dp_write_cache_desc - fill the dma control buf for
 *	dual plan cache programe plane_1 or plane_2
 *
 * @param info - nand info
 * @param page_addr
 * @param phys_data_addr - dma addr of the data buf
 * @param phys_data_addr - dma addr of the oob buf
 * @param cache - cache or uncache for this operation.
 *
 * with cache option, we have to wait the plane 1 ready and then start the second
 *	plane DMA, so can't put the two plane in one descriptor buffer, and this
 *	function is for the first plane. mainly to fill the page/data buffer/oob buffer.
 */
static void mv88dexx_nand_dma_fill_dp_write_cache_desc(struct pxa3xx_nand_info *info,
		int page_addr, dma_addr_t phys_data_addr, dma_addr_t phys_oob_addr,
		unsigned char* desc_buf, int cmd)
{
	...
	if (cmd) {
		...
		i += sizeof(SIE_BCMSEMA);
		cmd_3 = (struct cmd_3_desc *)(desc_buf + i);
		cmd_3->cfgw_ndcb0[0].u_dat = (cs ? NDCB0_CSEL : 0) |
			NDCB0_CMD_XTYPE(NDCB0_CMDX_NWRITE) |
			NDCB0_AUTO_RS |
			NDCB0_CMD_TYPE(NDCB0_CMD_NAKED_CMD) |
			addr_cycle |
...
/**
 * mv88dexx_nand_dma_init_dp_wr_cache_desc - initthe dma control buf for dual
 *	plan cache programe plane 1 or plane 2
 *
 * @param info - nand info
 *
 * with cache option, we have to wait the plane 1 ready and then start the second plane,
 * so can't put the two plane in one descriptor buffer, and this function is to init descriptor
 * buffer for both plane_1 and plane_2.
 */
static int mv88dexx_nand_dma_init_dp_wr_cache_desc(struct pxa3xx_nand_info *info,
		unsigned char *desc_buf, int px)
{
	struct pxa3xx_nand *nand = info->nand_data;
	unsigned int ready, cmd_done, page_done, badblock_detect;
	int addr_cycle, i = 0, c;
	...
	if (cs) {
		...
		page_done	= NDSR_CS1_PAGED;
		badblock_detect	= NDSR_CS1_BBD;
	} else {
		ready		= NDSR_FLASH_RDY;
		cmd_done	= NDSR_CS0_CMDD;
		page_done	= NDSR_CS0_PAGED;
		badblock_detect	= NDSR_CS0_BBD;
	}
	cmd		= info->cmdset->dual_plane_wr_cache;
	ndcb0_cs	= (cs ? NDCB0_CSEL : 0);
	mtd		= get_mtd_by_info(info);
	...
	nand->ecc_strength = info->ecc_strength;
	pxa3xx_set_datasize(info);
	chunks = (info->page_size + PAGE_CHUNK_SIZE - 1) / PAGE_CHUNK_SIZE;
	memset(desc_buf, 0, DMA_H_SIZE);
	...
	printk("DMA dp write cache p%d descriptor length = 0x%x\n", px, i);
	return i;
}
...
static void mv88dexx_nand_dma_dhub_kick(struct pxa3xx_nand *nand, dma_addr_t desc_addr, int desc_len)
{
	wmb();
	dhub_channel_write_cmd(
			&nand->PB_dhubHandle,		/* Handle to HDL_dhub */
			NFC_DEV_CTL_CHANNEL_ID,		/* Channel ID in $dHubReg */
			desc_addr,	/* CMD: buffer address */
			desc_len,	/* CMD: number of bytes to transfer */
			0,	/* CMD: semaphore operation at CMD/MTU (0/1) */
			0,	/* CMD: non-zero to check semaphore */
			0,	/* CMD: non-zero to update semaphore */
			1,	/* CMD: raise interrupt at CMD finish */
...
static void mv88dexx_nand_wait_1p_finish(struct pxa3xx_nand *nand)
{
	int retry, ndsr;
	unsigned long t, ret;
	...
	for (retry = 0; retry < 4; retry++) {
		ret = wait_for_completion_timeout(&nand->cmd_complete, t);
		if (!ret) {
			...
			if (retry)
				printk(KERN_INFO "%s pid %d "
						"survived, retry %d times\n",
						__func__, current->pid, retry);
			break;
		}
	}
	ndsr = nand_readl(nand, NDSR)|nand->reg_ndsr[nand->chip_select];
...
#endif
		/* Comment 18046:
		 * After this time out issue happens, following write operation may corrupt
		 * orignal data. Till now, we think the time out case is a HW issue. Also it is
		 * not recoverable by software operation. So now, block nand dirver until
		 * the ISR back to normal.
		 */
		printk(KERN_ERR "System ERROR, BLOCKING NAND DRIVER!!!\n");
	...
	{
		ndsr |= nand_readl(nand, NDSR);
	} while (!(ndsr & NDSR_FLASH_RDY) );
	nand_writel(nand, NDSR, ndsr);
	nand->reg_ndsr[nand->chip_select] = 0;
}

/**
 * mv88dexx_nand_dma_dp_program_wait_done - wait for the whole DMA data transmission
 *	for cache program to be finish.
 *
 * @param nand - nand pointer
 * @param command - command pass to nand driver
 *
 */
static int mv88dexx_nand_dma_dp_program_wait_done(struct pxa3xx_nand *nand,  unsigned int command)
{
	if (nand->cache_program_en && ((command == NAND_CMD_NAND_CACHE_FLUSH) || (command == NAND_CMD_PAGEPROG))) {
		int read;

		read = nand_readl(nand, NDSR)|nand->reg_ndsr[nand->chip_select];
		do
		{
			read |= nand_readl(nand, NDSR);
		} while ((read & NDSR_FLASH_RDY) != NDSR_FLASH_RDY);
		nand_writel(nand, NDSR, read);

		do
		{
			read = nand_readl(nand, NDCR);
		} while (read & NDCR_ND_RUN);
	}
	return 0;
}

/**
 * mv88dexx_nand_dma_dp_program -write data to nand,
 *		cache or no-cache is auto selected according to page.
 *
 * @param nand - nand pointer
 * @param command - command passed to nand driver
 *		NAND_CMD_NAND_CACHE_FLUSH: flush the last page data in buffer to nand.
 *
 */
static void mv88dexx_nand_dma_dp_program(struct pxa3xx_nand_info *info,  unsigned int command)
{
	struct pxa3xx_nand *nand = info->nand_data;
	unsigned int page_addr;
	int cs = nand->chip_select, cache = 0, flush = 0, cmd;
	uint32_t block_mask, last_idx, wr_idx;

	del_timer_sync(&nand->flush_timer);
	if (NAND_CMD_NAND_CACHE_FLUSH == command)
		flush = 1;
	/* choose cache or no-cache operation by current page number and last page number */
	block_mask = (~((1<<(info->erase_shift)) - 1))>>info->page_shift;
	wr_idx = nand->wr_idx[cs];
	if (likely(!flush)) {
		last_idx =  wr_idx ? (wr_idx -1) : (NUM_INTER_BUF-1);
		if ((block_mask & nand->page_addr) ==
			(block_mask&nand->wr_page[cs][last_idx])) {
			cache = 1;
			if (nand->buf_depth[cs] < (NUM_INTER_BUF-1))
				nand->buf_depth[cs]++;
		} else {
			cache = 0;
			nand->buf_depth[cs] = 1;
	...
	page_addr = nand->wr_page[cs][wr_idx];
	nand->data_buff_phys = map_addr(nand, (void *)nand->last_wr_buf[cs][wr_idx],
				NAND_MAX_IOSIZE,
				DMA_TO_DEVICE);
	nand->oob_buff_phys = nand->data_buff_phys + NAND_MAX_PAGESIZE;
	nand->reg_ndsr[nand->chip_select] = 0;
	page_addr = ((page_addr & (~0x7f)) << 1) + (page_addr & 0x7f);

	/* program the first plane */
	mv88dexx_nand_dma_fill_dp_write_cache_desc(info,
			page_addr, nand->data_buff_phys, nand->oob_buff_phys,
			(unsigned char *)(nand->dp_write_cache_desc1 + cs * DMA_H_SIZE),
			0);

	mv88dexx_nand_dma_dhub_kick(nand,
			nand->dp_write_desc_cache_addr1 + nand->chip_select * DMA_H_SIZE,
			nand->dp_write_cache_desc_len_p1[nand->chip_select]);

	/* program the second plane */
	cmd = cache ? info->cmdset->dual_plane_wr_cache : info->cmdset->dual_plane_write;
	mv88dexx_nand_dma_fill_dp_write_cache_desc(info, page_addr + 0x80,
			nand->data_buff_phys + nand->data_size*(nand->total_cmds - 1),
			nand->oob_buff_phys,
			(unsigned char *)(nand->dp_write_cache_desc2 + cs * DMA_H_SIZE),
			cmd);

	mv88dexx_nand_wait_1p_finish(nand);
	init_completion(&nand->cmd_complete);

	mv88dexx_nand_dma_dhub_kick(nand,
			nand->dp_write_desc_cache_addr2 + nand->chip_select * DMA_H_SIZE,
			nand->dp_write_cache_desc_len_p2[nand->chip_select]);
	unmap_addr(&nand->pdev->dev, nand->data_buff_phys,
		nand->last_wr_buf[cs][wr_idx],
		NAND_MAX_IOSIZE,
		DMA_TO_DEVICE);

	if (likely(!flush)) {
		if (cache) {
			nand->last_wrtime[cs] = ktime_get();
			nand->flush_timer.expires = jiffies + NAND_CACHE_FLUSH_TIMEOUT;
			add_timer(&nand->flush_timer);
		}
	} else {
		int i;
		nand->buf_depth[cs]	= 0;
		nand->wr_idx[cs]		= 0;
		for (i = 0; i < NUM_INTER_BUF; i++) {
			nand->wr_page[cs][i]	= INVALID_PAGE;
		}
	}

}

static int  pxa3xx_nand_cmd_pre_check(struct pxa3xx_nand_info *info, unsigned int cmd,
		int page_addr)
{
	struct pxa3xx_nand *nand = info->nand_data;
	int ret = 0;
	if (nand->cache_program_en && (info->n_planes == 2)) {
		int cs = nand->chip_select;
		switch (cmd) {
		case NAND_CMD_ERASE1:
		{
			uint32_t block_mask;
			block_mask = (~((1<<(info->erase_shift)) - 1))>>info->page_shift;
			if (nand->buf_depth[cs] &&
				((block_mask & page_addr) ==
				(block_mask & nand->wr_page[cs][nand->wr_idx[cs]]))) {
				/* in the same block, just drop the page */
				nand->wr_idx[cs]		= 0;
				nand->wr_page[cs][0]	= INVALID_PAGE;
				nand->buf_depth[cs]	= 0;
			}
			break;
		}
		case NAND_CMD_PAGEPROG:
			nand->new_progam_cmd[cs] = 1;
			if (nand->buf_depth[cs] == 0) {
				nand->buf_depth[cs]++;
				nand->skip_following_status_read[cs] = true;
				nand->flush_timer.expires = jiffies + NAND_CACHE_FLUSH_TIMEOUT;
				add_timer(&nand->flush_timer);
				ret = -1;
			}
		...
		if (ret != -1) {
			nand->skip_following_status_read[cs] = false;
		}
	}
	return ret;
}

static int pxa3xx_nand_find_cached_page(struct pxa3xx_nand *nand, int page_addr)
{
	int i, idx;
	int cs = nand->chip_select;

	idx = nand->wr_idx[cs];
	for (i=0; i< nand->buf_depth[cs]; i++) {

		if (nand->wr_page[cs][idx] == page_addr) {
			return idx;
		}
		if (idx) {
			idx--;
		...
		unsigned command)
{
	...
	if (command == NAND_CMD_NAND_CACHE_FLUSH) {
		if (nand->cache_program_en && info->n_planes == 2) {
	...
	case NAND_CMD_RNDOUT:
		if (info->n_planes == 2) {
			...
			if (!ret) {
				...
				pb_init(nand);
#endif
				/* Block nand driver, same reason as above
				 * comment 18046
				 */
				printk(KERN_ERR "System ERROR, BLOCKING NAND DRIVER!!!\n");
				wait_for_completion(&nand->cmd_complete);
	...
	case NAND_CMD_PAGEPROG:
		if (info->n_planes == 2) {
			if (nand->cache_program_en) {
				mv88dexx_nand_dma_dp_program(info, command);
			} else {
	...
	if (info->n_planes == 2) {
		mv88dexx_nand_dma_dp_program_wait_done(nand, command);
	}
...
#endif
	if (pxa3xx_nand_cmd_pre_check(info, command, page_addr) != 0) {
		return;
	}
...
#if (PXA3XX_NAND_DMA_DEBUG == 1)
	debug_push_staus(nand->status_rec|nand_readl(nand, NDSR));
#endif
		...
		struct nand_chip *chip, const uint8_t *buf)
{
	...
	if (info->n_planes == 2) {
		...
		if (nand->cache_program_en) {
			int cs = nand->chip_select;
			int wr_idx = nand->wr_idx[cs];
			if(nand->new_progam_cmd[cs]){
				wr_idx = (wr_idx < (NUM_INTER_BUF - 1))  ? (wr_idx+1) : 0;
				nand->wr_idx[cs] = wr_idx;
			}
			nand->wr_page[cs][wr_idx] = nand->page_addr;
			data_buf	= nand->last_wr_buf[cs][wr_idx];
			oob_buf	= nand->last_wr_oob_buf[cs][wr_idx];
			nand->new_progam_cmd[cs] = 0;
		} else {
			data_buf	= nand->bounce_buffer;
			oob_buf	= nand->bounce_buffer + mtd->writesize;
		}
		...
		if (nand_write_page_pre_process(mtd, page_addr,
			...
			nand_write_page_pre_process(mtd, page_addr + pages_per_blk,
					...
					data_buf + info->page_size, NULL);
			buf = data_buf;
			oob = oob_buf;
		}
		if (nand->cache_program_en) {
			nand->use_mapped_buf = 0;
			goto skip_map;
		}
	} else {
		...
		struct nand_chip *chip, const uint8_t *buf)
{
	...
	if (nand->cache_program_en) {
		...
		memcpy(nand->last_wr_buf[cs][wr_idx],
			buf, mtd->writesize);
		memcpy(nand->last_wr_oob_buf[cs][wr_idx],
			oob, mtd->oobsize);
		nand->write_size[cs]	= mtd->writesize;
		nand->write_oob_size[cs] = mtd->oobsize;
		nand->wr_page[cs][wr_idx] = nand->page_addr;
	}
...
static void pxa3xx_wait_cache_ready(struct pxa3xx_nand *nand)
{
	int time_out = 630;
	ktime_t delta;
	int cs = nand->chip_select;
	...
	delta = ktime_sub(ktime_get(), nand->last_wrtime[cs]);
...
/**
 * pxa3xx_page_pre_read - when cache is enable, the last page write to nand is
 *	buffered in ram, and also the ( last -1)th page data maybe still in nand
 *	interal cache, so we return these data from buffer directly not read it
 *	from cache. and also both single plane and dual plane operation to the
 *	same nand need to wait cache ready.
 *
 *
 * @param mtd - mtd info structure
 * @param buf - read buffer
 * @param page - read page number
 *
 * return:
 * 	1:	- read page is in nand internal buffer, copy it back and don't need
 *		real nand read.
 *	0:	- do normal nand read operation
 */
static int pxa3xx_page_pre_read(struct mtd_info *mtd, uint8_t *buf, int page)
{
	struct pxa3xx_nand_info *info = mtd->priv;
	struct pxa3xx_nand *nand = info->nand_data, *nand_dp = NULL;
	int cs = nand->chip_select;

	if (nand->cache_program_en) {
		if (info->n_planes == 2) {
			int idx;
			if (buf && ((idx = pxa3xx_nand_find_cached_page(nand, page)) != -1)) {
				//nand_debug_gbx1[2] = idx;
				if (nand->wr_page[cs][idx] != INVALID_PAGE) {
					uint64_t *p1, *p2;
					struct nand_chip *chip = mtd->priv;
					nand->data_buff = buf;
					nand->oob_buff = chip->oob_poi;
#ifdef CONFIG_MV88DE3100_NAND_RANDOMIZER
					page = ((page & (~0x7f)) << 1) + (page & 0x7f);
					//page is buffered and not write to flash
					nand_read_page_post_process(mtd, page,
							nand->last_wr_buf[cs][idx],
							nand->last_wr_oob_buf[cs][idx],
							buf,
							nand->oob_buff);
					nand_read_page_post_process(mtd, page + 0x80,
							nand->last_wr_buf[cs][idx] + info->page_size,
							NULL,
							buf + info->page_size,
							NULL);
#else
					memcpy(buf, nand->last_wr_buf[cs][idx], mtd->writesize);
					memcpy(nand->oob_buff, nand->last_wr_oob_buf[cs][idx],
						nand->write_oob_size);
#endif
					...
					*p2++ = *p1++;
					...
					printk("NAND cache ERRO: page %d, depth %d  !!!!!!\n",
						page, nand->buf_depth[cs]);
				}

			}

			if (nand->buf_depth[cs] > 1) {
				nand_dp = nand;
			}

		} else {
			struct pxa3xx_nand_info *info_dp;
			info_dp = (nand->info_dp[nand->chip_select]);
			nand_dp = info_dp->nand_data;
			if (nand_dp && (!nand_dp->cache_program_en ||nand->buf_depth[cs] < 2)) {
				nand_dp = NULL;
			}
		}

		if (nand_dp) {
			pxa3xx_wait_cache_ready(nand_dp);
		}
	}
...
static int pxa3xx_read_page(struct mtd_info *mtd, uint8_t *buf, int page)
{
	...
	if (pxa3xx_page_pre_read(mtd, buf, page) == 1) {
		return 0;
	}
	if (use_dma) {
again:
	...
	if (nand->retcode == ERR_SBERR) {
		 ...
		 */
		if (is_read_datbuf_blank(nand->data_buff, mtd->writesize)) {
...
static void pxa3xx_nand_flush_device(struct mtd_info *mtd)
{
	nand_get_device(mtd, FL_WRITING);
	pxa3xx_nand_cmdfunc(mtd, NAND_CMD_NAND_CACHE_FLUSH, 0, 0);
	nand_release_device(mtd);
}

/**
 * pxa3xx_nand_flush_need - for uplayer to check whether need flush before ommit a read
 *	or write operation.
 *
 * @param mtd - current operation mtd dev info
 * @param command - command pass to nand driver
 *
 * return - return NULL if flush is not needed, or else return the MTD pointer to flush.
 */
static void* pxa3xx_nand_flush_need(struct mtd_info *mtd, unsigned command, int column, int page_addr)
{
	struct pxa3xx_nand_info *info = mtd->priv;
	struct pxa3xx_nand *nand = info->nand_data;
	...
	if (nand->cache_program_en){
		if (info->n_planes == 2) {
			...
			case NAND_CMD_RNDOUT:
			case NAND_CMD_READ0:
				if(nand->buf_depth[nand->chip_select])
					return (void *)mtd;
				break;
			default:
			break;
		...
		if (info->n_planes == 1) {
			struct mtd_info *mtd_dp;
			mtd_dp = get_mtd_by_info(nand->info_dp[info->chip_select]);
			switch (command) {
			case NAND_CMD_PAGEPROG:
			case NAND_CMD_RNDOUT:
				if(nand->buf_depth[nand->chip_select])
...
static int pxa3xx_nand_waitfunc(struct mtd_info *mtd, struct nand_chip *this)
{
	...
	if (nand->cache_program_en && nand->skip_following_status_read[nand->chip_select]) {
		nand->skip_following_status_read[nand->chip_select] = false;
		return 0;
	}
...
static int pxa3xx_nand_scan(struct mtd_info *mtd, int type_plane)
{
	...
	if (use_dma) {
		if (type_plane == TYPE_DUAL_PLANE) {
			int cs = info->chip_select;
			mv88dexx_nand_dma_init_dp_write_desc(info);
			nand->dp_write_cache_desc_len_p1[cs] =
				mv88dexx_nand_dma_init_dp_wr_cache_desc(info,
					nand->dp_write_cache_desc1 + cs * DMA_H_SIZE,
					1);
			nand->dp_write_cache_desc_len_p2[info->chip_select] =
				mv88dexx_nand_dma_init_dp_wr_cache_desc(info,
					nand->dp_write_cache_desc2 + cs * DMA_H_SIZE,
					2);
		} else {
...
static int pxa3xx_dp_nand_cache_flush_thread(void *data)
{
	struct pxa3xx_nand *nand = (struct pxa3xx_nand *)data;
	struct mtd_info *mtd_dp;
	...
	while (!kthread_should_stop()) {
		...
		wait_event_interruptible(nand->flush_event,
				kthread_should_stop() ||
				try_to_freeze() ||
				nand->flush_needed);

		if (nand->flush_needed) {
			__set_current_state(TASK_RUNNING);
			mtd_dp = get_mtd_by_info(nand->info_dp[nand->chip_select]);
			pxa3xx_nand_flush_device(mtd_dp);
			nand->flush_needed = 0;
		}
...
static void pxa3xx_dp_nand_cache_flush_timer(unsigned long data)
{
	struct pxa3xx_nand *nand = (struct pxa3xx_nand *)data;
	if (nand->cache_program_en && nand->buf_depth[nand->chip_select]) {
		if (!nand->flush_needed) {
			nand->flush_needed = 1;
			wake_up(&nand->flush_event);
		}
	}
}

static void pxa3xx_dp_init_nand_cache_flush_timer(struct pxa3xx_nand *nand)
{
	struct platform_device *pdev = nand->pdev;
	struct pxa3xx_nand_platform_data *pdata;

	pdata = pdev->dev.platform_data;
	init_timer(&nand->flush_timer);
	nand->flush_timer.function = pxa3xx_dp_nand_cache_flush_timer;
	nand->flush_timer.data = (unsigned long)nand;
	nand->flush_thread = kthread_create(pxa3xx_dp_nand_cache_flush_thread, nand,
			"nand_flush_task");
	nand->flush_needed = 0;
	init_waitqueue_head(&nand->flush_event);
	if (IS_ERR(nand->flush_thread)) {
		printk("ERROR!!!, creat flush thread failed, run without cache\n");
		nand->cache_program_en = 0;
		del_timer_sync(&nand->flush_timer);
		kfree(nand->last_wr_buf[0][0]);
	} else {
		wake_up_process(nand->flush_thread);
	}
}

static void pxa3xx_dp_deinit_nand_cache_flush_timer(struct pxa3xx_nand *nand)
{
	del_timer_sync(&nand->flush_timer);
	nand->flush_needed = 0;
	if (!IS_ERR(nand->flush_thread)) {
		kthread_stop(nand->flush_thread);
		nand->flush_thread = NULL;
	}
}
...
static int alloc_nand_resource(struct platform_device *pdev)
{
	...
	if (use_dma) {
		...
		nand->dp_write_cache_desc1 = nand->dp_write_desc
			+ DMA_H_SIZE*pdata->cs_num;
		nand->dp_write_cache_desc2 = nand->dp_write_cache_desc1
			+ DMA_H_SIZE*pdata->cs_num;
		...
		nand->dp_write_desc_addr	= nand->dp_read_desc_addr
			+ DMA_H_SIZE * pdata->cs_num;
		nand->dp_write_desc_cache_addr1	= nand->dp_write_desc_addr
			+ DMA_H_SIZE*pdata->cs_num;
		nand->dp_write_desc_cache_addr2	= nand->dp_write_desc_cache_addr1
			+ DMA_H_SIZE*pdata->cs_num;
	}

	/* cache program related alloc */
	nand->cache_program_en = use_cache_program;
	if (nand->cache_program_en) {
		printk("nand cache for dp enable, nand %p, dev %p\n", nand, pdev);
		nand->last_wr_buf[0][0] = kmalloc(NUM_INTER_BUF * NAND_MAX_IOSIZE * pdata->cs_num,
					GFP_KERNEL);
		if (nand->last_wr_buf[0][0] == NULL) {
			dev_err(&pdev->dev, "failed to allocate"
					" dma cache buffer\n");
			ret = -ENOMEM;
			goto fail_free_write_desc;
		}
		nand->last_wr_oob_buf[0][0]         =  nand->last_wr_buf[0][0]  + NAND_MAX_PAGESIZE;
		for (i=1; i<NUM_INTER_BUF; i++) {
			nand->last_wr_buf[0][i]		= nand->last_wr_buf[0][i-1]
								+ NAND_MAX_IOSIZE;
			nand->last_wr_oob_buf[0][i]	= nand->last_wr_oob_buf[0][i-1]
								+ NAND_MAX_IOSIZE;
		}
		for (cs=1; cs<pdata->cs_num; cs++) {
			nand->last_wr_buf[cs][0]		= nand->last_wr_buf[cs-1][0]
								+ NAND_MAX_IOSIZE * NUM_INTER_BUF;
			nand->last_wr_oob_buf[cs][0]	=  nand->last_wr_oob_buf[cs-1][0]
								+ NAND_MAX_IOSIZE * NUM_INTER_BUF;
			for (i=1; i<NUM_INTER_BUF; i++) {
				nand->last_wr_buf[cs][i]		= nand->last_wr_buf[cs][i-1]
									+ NAND_MAX_IOSIZE;
				nand->last_wr_oob_buf[cs][i]	=  nand->last_wr_oob_buf[cs][i-1]
									+ NAND_MAX_IOSIZE;
			}
		}
	}
	...
	for (cs=0; cs<pdata->cs_num; cs++) {
		...
		nand->wr_idx[cs]		= 0;
		nand->write_size[cs]	= 0;
		nand->write_oob_size[cs]	= 0;
		nand->buf_depth[cs]	= 0;
		nand->last_wrtime[cs].tv64	= 0;
		nand->new_progam_cmd[cs] = 0;
		nand->skip_following_status_read[cs] = false;
	}
	...
	for (cs = 0; cs < pdata->cs_num; cs++) {
		...
		chip->mtd_flush_need	= pxa3xx_nand_flush_need;
...
static int pxa3xx_nand_remove(struct platform_device *pdev)
{
	...
	if (nand->cache_program_en) {
		pxa3xx_dp_deinit_nand_cache_flush_timer(nand);
		kfree(nand->last_wr_buf[0][0]);
	}
...
add_predefined_parts:
	...
	if (!probe_success) {
		...
		if (nand->cache_program_en)
			pxa3xx_dp_init_nand_cache_flush_timer(nand);
		return 0;
	}
}
