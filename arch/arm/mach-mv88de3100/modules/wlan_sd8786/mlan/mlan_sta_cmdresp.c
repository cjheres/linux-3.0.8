                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    for (i = 1; i <= MAX_PORT - pmadapter->mp_end_port; i++) {
                                ...
                                t_u32 upld_len)
{
    ...
    pmlan_private pmpriv = wlan_get_priv(pmadapter, MLAN_BSS_TYPE_STA);
    ...
    if (!pmpriv)
        pmpriv = wlan_get_priv(pmadapter, MLAN_BSS_TYPE_STA);
    ...
    PRINTM(MCMND, "Sleep confirm resp\n");
    ...
    if (pmadapter->is_hs_configured && !pmadapter->sleep_period.period) {
        wlan_host_sleep_activated_event(wlan_get_priv
                                        (pmadapter, MLAN_BSS_TYPE_STA), MTRUE);
    }
    pmadapter->ps_state = PS_STATE_SLEEP;
    LEAVE();
}
                         ...
                         IN t_void * pcmd_buf, IN t_void * pioctl)
{
    ...
    case HostCmd_CMD_RECONFIGURE_TX_BUFF:
        pmadapter->tx_buf_size = wlan_le16_to_cpu(resp->params.
                                                  tx_buf.buff_size);
