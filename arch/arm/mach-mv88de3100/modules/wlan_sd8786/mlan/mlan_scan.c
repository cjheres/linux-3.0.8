                              ...
                              IN t_u8 filtered_scan)
{
    ...
    for (region_idx = 0;
        ...
        for (next_chan = 0;
            ...
            if (wlan_11d_get_state(pmpriv) == ENABLE_11D)
                scan_type = wlan_11d_get_scan_type(pmadapter, cfp->channel,
                                                   &pmadapter->
                                                   parsed_region_chan);
            ...
            if (puser_scan_in && puser_scan_in->chan_list[0].scan_time) {
                pscan_chan_list[chan_idx].max_scan_time =
                    wlan_cpu_to_le16(puser_scan_in->chan_list[0].scan_time);
            ...
            pscan_chan_list[chan_idx].chan_number = cfp->channel;
                       ...
                       IN ChanScanParamSet_t * pscan_chan_list)
{
    ...
    while (ptmp_chan_list->chan_number) {
        ...
        while (tlv_idx < max_chan_per_scan && ptmp_chan_list->chan_number &&
             ...
             */
            pscan_cfg_out->tlv_buf_len = ((t_u8 *) pchan_tlv_out
    ...
    *pscan_current_only = MFALSE;
    ...
    if (puser_scan_in) {
        ...
        pscan_cfg_out->bss_mode =
            (puser_scan_in->bss_mode ? puser_scan_in->bss_mode : pmadapter->
             scan_mode);
        ...
        pscan_cfg_out->bss_mode = pmadapter->scan_mode;
    ...
    if (num_probes) {
        ...
        pnum_probes_tlv->num_probes = wlan_cpu_to_le16(num_probes);
    ...
    if (puser_scan_in && puser_scan_in->chan_list[0].chan_number) {
        ...
        for (chan_idx = 0;
            ...
            if (puser_scan_in->chan_list[chan_idx].scan_time) {
                scan_dur = puser_scan_in->chan_list[chan_idx].scan_time;
            } else {
                if (scan_type == HostCmd_SCAN_TYPE_PASSIVE) {
    ...
    while (bytes_left_for_current_beacon >= 2) {
        ...
        case VENDOR_SPECIFIC_221:
            ...
            if (!memcmp(pvendor_ie->vend_hdr.oui, wpa_oui, sizeof(wpa_oui))) {
                ...
                pbss_entry->wpa_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
                HEXDUMP("InterpretIE: Resp WPA_IE",
                        (t_u8 *) pbss_entry->pwpa_ie,
                        ((*(pbss_entry->pwpa_ie)).vend_hdr.len
                         + sizeof(IEEEtypes_Header_t)));
            } else if (!memcmp(pvendor_ie->vend_hdr.oui,
                ...
                pbss_entry->pwps_ie =
                    (IEEEtypes_VendorSpecific_t *) pcurrent_ptr;
                pbss_entry->wps_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
                HEXDUMP("InterpretIE: Resp WPS_IE",
                        (t_u8 *) pbss_entry->pwps_ie, total_ie_len);
            }
            break;
        case RSN_IE:
            pbss_entry->prsn_ie = (IEEEtypes_Generic_t *) pcurrent_ptr;
            pbss_entry->rsn_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
        ...
        case HT_CAPABILITY:
            ...
            pbss_entry->ht_cap_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
        ...
        case HT_OPERATION:
            ...
            pbss_entry->ht_info_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
        ...
        case EXT_CAPABILITY:
            ...
            pbss_entry->ext_cap_offset = pcurrent_ptr - pbss_entry->pbeacon_buf;
                                  ...
                                  IN BSSDescriptor_t * pnew_beacon)
{
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf) {
        ...
        if (new_bcn_size == old_bcn_size) {
            ...
            if (old_bcn_size < bcn_space && new_bcn_size <= old_bcn_size) {
                ...
                for (adj_idx = 0; adj_idx < num_of_ent; adj_idx++) {
                    if (pmadapter->pscan_table[adj_idx].pbeacon_buf >
                        ...
                        if (pmadapter->pscan_table[adj_idx].pwps_ie) {
                            pmadapter->pscan_table[adj_idx].pwps_ie =
                                (IEEEtypes_VendorSpecific_t *)
                                (pmadapter->pscan_table[adj_idx].pbeacon_buf +
                                 pmadapter->pscan_table[adj_idx].wps_offset);
                        }
            ...
            if (pnew_beacon->pwps_ie)
                pnew_beacon->wps_offset =
                    pmadapter->pscan_table[beacon_idx].wps_offset;
        ...
        if (pnew_beacon->pwps_ie) {
            pnew_beacon->pwps_ie = (IEEEtypes_VendorSpecific_t *)
                (pnew_beacon->pbeacon_buf + pnew_beacon->wps_offset);
        }
        if (pnew_beacon->pht_cap) {
        ...
        if (pmadapter->pbcn_buf_end + pnew_beacon->beacon_buf_size +
            ...
            pnew_beacon->pwps_ie = MNULL;
            pnew_beacon->wps_offset = 0;
...
wlan_scan_process_results(IN mlan_private * pmpriv)
{
    ...
    if (pmpriv->media_connected == MTRUE) {
        ...
        pmpriv->curr_bss_params.bss_descriptor.pwps_ie = MNULL;
        pmpriv->curr_bss_params.bss_descriptor.wps_offset = 0;
...
wlan_scan_delete_table_entry(IN mlan_private * pmpriv, IN t_s32 table_idx)
{
    ...
    for (del_idx = table_idx; (del_idx + 1) < pmadapter->num_in_scan_table;
        ...
        if (pmadapter->pscan_table[del_idx].pbeacon_buf) {
            ...
            if (pmadapter->pscan_table[del_idx].pwps_ie) {
                pmadapter->pscan_table[del_idx].pwps_ie =
                    (IEEEtypes_VendorSpecific_t *)
                    (pmadapter->pscan_table[del_idx].pbeacon_buf +
                     pmadapter->pscan_table[del_idx].wps_offset);
            }
                     ...
                     IN HostCmd_DS_COMMAND * pcmd, IN t_void * pdata_buf)
{
    ...
    pcmd->size = wlan_cpu_to_le16(sizeof(pscan_cmd->bss_mode)
                                  + sizeof(pscan_cmd->bssid)
