                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        if ((cfg->param.tx_buf_size == MLAN_TX_DATA_BUF_SIZE_2K) ||
            ...
            pmadapter->max_tx_buf_size = cfg->param.tx_buf_size;
        } else
            ret = MLAN_STATUS_FAILURE;
    } else
        cfg->param.tx_buf_size = pmadapter->max_tx_buf_size;
...
wlan_send_addba(mlan_private * priv, int tid, t_u8 * peer_mac)
{
    ...
    add_ba_req.block_ack_param_set = (tid << BLOCKACKPARAM_TID_POS) |
        (priv->add_ba_param.tx_win_size
         << BLOCKACKPARAM_WINSIZE_POS) | IMMEDIATE_BLOCK_ACK;
    add_ba_req.block_ack_tmo = priv->add_ba_param.timeout;
