                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    case 4:
        ...
        pg = (Power_Group_t *) (buf + sizeof(HostCmd_DS_TXPWR_CFG) +
                                sizeof(MrvlTypes_Power_Group_t));
        pg->modulation_class = mod_class;
        pg->first_rate_code = data[0];
        pg->last_rate_code = data[0];
        if (mod_class == MOD_CLASS_OFDM) {
            pg->first_rate_code = data[0] - MLAN_RATE_INDEX_OFDM0;
            pg->last_rate_code = data[0] - MLAN_RATE_INDEX_OFDM0;
        } else if (mod_class == MOD_CLASS_HT) {
            pg->first_rate_code = data[0] - MLAN_RATE_INDEX_MCS0;
            pg->last_rate_code = data[0] - MLAN_RATE_INDEX_MCS0;
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    wlan_prepare_cmd(pmpriv, HostCmd_CMD_FUNC_SHUTDOWN,
                     HostCmd_ACT_GEN_SET, 0, (t_void *) pioctl_req, MNULL);
