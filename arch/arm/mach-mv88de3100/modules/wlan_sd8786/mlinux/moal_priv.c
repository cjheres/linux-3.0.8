...
{
    t_u8 buf[7];
    t_u8 rw, func, data = 0xff;
    int reg;
#ifdef CONFIG_ARCH_MV88DE3100
    int ret;
    struct sdio_mmc_card *card;
#endif
    ENTER();
#ifdef CONFIG_ARCH_MV88DE3100
    card = (struct sdio_mmc_card *) priv->phandle->card;
#endif

    if (copy_from_user(buf, wrq->u.data.pointer, sizeof(buf))) {
        PRINTM(MINFO, "Copy from user failed\n");
        LEAVE();
        return -EFAULT;
    }
	...
	if (ret) {
            PRINTM(MINFO, "sdio_readb: reading register 0x%X failed\n",
                   reg);
#else
        if (sdio_read_ioreg(priv->phandle->card, func, reg, &data) < 0) {
            PRINTM(MINFO, "sdio_read_ioreg: reading register 0x%X failed\n",
		   reg);
#endif
            data = 0xff;
        }
	...
	if (ret) {
            PRINTM(MINFO, "sdio_writeb: writing register 0x%X failed\n",
                   reg);
#else
        if (sdio_write_ioreg(priv->phandle->card, func, reg, data) < 0) {
            PRINTM(MINFO, "sdio_read_ioreg: writing register 0x%X failed\n",
		   reg);
#endif
            data = 0xff;
        }
...
woal_cmd53rdwr_ioctl(moal_private * priv, struct iwreq *wrq)
{
    ...
    int reg = 0, ret = MLAN_STATUS_SUCCESS;
    t_u8 *data = NULL;
#ifdef CONFIG_ARCH_MV88DE3100
    struct sdio_mmc_card *card;
    card = (struct sdio_mmc_card *)priv->phandle->card;
    ...
    blknum =
        (mode == BLOCK_MODE) ? (blklen / MLAN_SDIO_BLOCK_SIZE) + 1 : blklen;
    PRINTM(MINFO,
    ...
    if (!rw) {
#ifdef CONFIG_ARCH_MV88DE3100
        if (sdio_readsb(card->func, data, reg,
                            blknum*blklen) < 0)
        ...
        memset(data, 0, sizeof(data));
        memcpy(data, &buf[11], (wrq->u.data.length - 11));
#ifdef CONFIG_ARCH_MV88DE3100
        if (sdio_writesb(card->func, reg, data,
                             blknum*blklen ) < 0)
...
woal_set_get_ps_cfg(moal_private * priv, struct iwreq *wrq)
{
    ...
    if (wrq->u.data.length) {
        ...
        if (wrq->u.data.length < allowed)
            data[5] = DELAY_TO_PS_UNCHANGED;
        else if ((data[5] < MIN_DELAY_TO_PS) || (data[5] > MAX_DELAY_TO_PS)) {
