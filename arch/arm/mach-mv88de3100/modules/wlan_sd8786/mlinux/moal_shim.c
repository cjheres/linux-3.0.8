                     ...
                     IN pmlan_buffer pmbuf, IN t_u32 port, IN t_u32 timeout)
{
    ...
    if ( unlikely(atomic_read(&handle->int_flag) == -100) ) {
	    printk("%s: boot firmware command\n");
    }
    else if ( unlikely( !(atomic_read(&handle->int_flag) ) ) )
    {
	 printk("%s: get intflag here: %d\n", __func__, atomic_read(&handle->int_flag));
	 locked ++;
//	 sdio_claim_host(card->func);
    }
    else{
        if (atomic_read(&card->in_int)) {
            printk ("%s: He Wu card -> in_int_flag = %d wait...\n", __func__, atomic_read( &card->in_int));
	    locked ++;
	    sdio_claim_host(card->func);
        }
    }
    ret = woal_write_data_sync((moal_handle *) pmoal_handle, pmbuf, port,
                                timeout);
                    ...
                    IN OUT pmlan_buffer pmbuf, IN t_u32 port, IN t_u32 timeout)
{
    ...
    if ( unlikely( !atomic_read( &handle->int_flag) ) )
    {
	locked ++;
	printk("%s: get intflag here: %d\n", __func__, atomic_read(&handle->int_flag));
        sdio_claim_host(card->func);
    }
    ret = woal_read_data_sync((moal_handle *) pmoal_handle, pmbuf, port,
                               timeout);
    if ( locked )
    {
	sdio_release_host(card->func);
...
moal_write_reg(IN t_void * pmoal_handle, IN t_u32 reg, IN t_u32 data)
{
    ...
    t_u32 locked = 0;
    card = handle->card;
    if ( likely( !atomic_read( &handle->int_flag)) )
    {
	locked ++;
	printk("%s: get intflag here: %d\n", __func__, atomic_read(&handle->int_flag));
        sdio_claim_host(card->func);
    }
    ret = woal_write_reg((moal_handle *) pmoal_handle, reg, data);
...
moal_read_reg(IN t_void * pmoal_handle, IN t_u32 reg, OUT t_u32 * data)
{
    ...
    ret = woal_read_reg((moal_handle *) pmoal_handle, reg, data);
    if ( locked )
    {
	sdio_release_host(card->func);
    }
...
mlan_status
moal_get_cardregs(IN t_void * pmoal_handle, IN t_u8 * regs)
{
    return woal_get_cardregs((moal_handle *) pmoal_handle, regs);
}
