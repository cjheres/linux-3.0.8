...
#define DFS_JAPAN_RADAR_CHANNEL_START    36
 ...
 */
int
check_channel_validity_11d(int channel, int set_domain)
{
...
apcmd_sys_cfg_channel(int argc, char *argv[])
{
    ...
    if ((argc == 1) || ((argc == 2) && !(atoi(argv[1]) & BITMAP_ACS_MODE))) {
        ...
        if (state_80211d) {
            ret = check_channel_validity_11d(atoi(argv[0]), 1);
            if (ret == UAP_FAILURE) {
                print_sys_cfg_channel_usage();
                return ret;
            }
...
apcmd_sys_cfg_scan_channels(int argc, char *argv[])
{
    ...
    if (argc) {
        ...
        if (state) {
            for (i = 0; i < argc; i++) {
                ret = check_channel_validity_11d(atoi(argv[i]), 0);
                if (ret == UAP_FAILURE) {
                    print_sys_cfg_channel_usage();
                    return UAP_FAILURE;
                }
...
apcmd_sta_filter_table(int argc, char *argv[])
{
    ...
    if (argc == 0) {
        ...
        for (i = 0; i < tlv->count; i++) {
            if ((ret = mac2raw(argv[i + 1], &tlv->mac_address[i * ETH_ALEN])) !=
                UAP_SUCCESS) {
                printf("ERR: %s Address\n",
