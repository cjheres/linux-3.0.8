...
apcmd_cmd52_readwrite(int argc, char *argv[])
{
    ...
    printf("func=%d, reg=%d, data=0x%02X\n",
           param.cmd52_params[0], param.cmd52_params[1], param.cmd52_params[2]);
    /* Close socket */
    close(sockfd);
    return UAP_SUCCESS;
}
 ...
 *
 *  @param argc     Pointer to structure tx_bf_cfg_para
 *
 *  $return         N/A
 */
void
print_tx_bf_response(tx_bf_cfg_para * param)
{
    ...
    no_tx_args *bf_no_tx = param->body.bf_no_tx;
    ...
    case SET_GET_BF_PERIODICITY:
        printf("TX BF Interval (in ms) = %ld\n", param->body.interval);
        break;
    case NO_TX_BF_FOR_PEER:
        for (i = 0; i < param->no_of_peers; i++) {
            printf("PEER MAC[%d] = %02X:%02X:%02X:%02X:%02X:%02X\n", i,
                   bf_no_tx->peer_mac[0], bf_no_tx->peer_mac[1],
                   bf_no_tx->peer_mac[2], bf_no_tx->peer_mac[3],
                   bf_no_tx->peer_mac[4], bf_no_tx->peer_mac[5]);
            bf_no_tx++;
        }
        break;
    case SET_SNR_THR_PEER:
        for (i = 0; i < param->no_of_peers; i++) {
            printf("PEER MAC[%d] = %02X:%02X:%02X:%02X:%02X:%02X, SNR = %d\n",
                   i, bf_snr->peer_mac[0], bf_snr->peer_mac[1],
                   bf_snr->peer_mac[2], bf_snr->peer_mac[3],
                   bf_snr->peer_mac[4], bf_snr->peer_mac[5], bf_snr->snr);
            bf_snr++;
        }
        break;
    }
...
apcmd_sys_cfg_tx_bf(int argc, char *argv[])
{
    ...
    if ((IS_HEX_OR_DIGIT(argv[0]) == UAP_FAILURE) ||
        ((atoi(argv[0]) < TRIGGER_SOUNDING_FOR_PEER) ||
    ...
    if (argc > 1) {
        ...
        case TRIGGER_SOUNDING_FOR_PEER:
            ...
            if (IS_HEX_OR_DIGIT(argv[2]) == UAP_FAILURE)
                return UAP_FAILURE;
            param.body.bf_sound[0].status = (t_u8) A2HEXDECIMAL(argv[2]);
            break;
        case SET_GET_BF_PERIODICITY:
            if (argc != 2) {
                printf("ERR: wrong arguments.\n");
            ...
            if (IS_HEX_OR_DIGIT(argv[1]) == UAP_FAILURE)
                return UAP_FAILURE;
            param.body.interval = (t_u32) A2HEXDECIMAL(argv[1]);
            break;
        case NO_TX_BF_FOR_PEER:
            if (argc != 2) {
                printf("ERR: wrong arguments.\n");
            ...
            if ((ret =
                 mac2raw(argv[1],
                         param.body.bf_no_tx[0].peer_mac)) != UAP_SUCCESS) {
                printf("ERR: %s Address\n",
                       ret == UAP_FAILURE ? "Invalid MAC" : ret ==
                       UAP_RET_MAC_BROADCAST ? "Broadcast" : "Multicast");
                return UAP_FAILURE;
        ...
        case SET_SNR_THR_PEER:
            ...
            if ((ret =
                 mac2raw(argv[1],
                         param.body.bf_snr[0].peer_mac)) != UAP_SUCCESS) {
                printf("ERR: %s Address\n",
                       ret == UAP_FAILURE ? "Invalid MAC" : ret ==
                       UAP_RET_MAC_BROADCAST ? "Broadcast" : "Multicast");
                return UAP_FAILURE;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (is_ap_mac_filter == 1) {
            if (strcmp(args[0], "}") == 0) {
                ...
                if (filter_tlv->filter_mode && (filter_tlv->count == 0)) {
                    printf
                        ("ERR:Filter list can not be empty for %s Filter mode\n",
                         (filter_tlv->filter_mode ==
                          1) ? "'Allow'" : "'Block'");
                    ret = UAP_FAILURE;
                    goto done;
                }
    ...
    if (ret == UAP_SUCCESS) {
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            while ((unsigned int) size >= sizeof(tlvbuf_header)) {
                ...
                case 2:
                    printf("%d",
                           (unsigned int) uap_le16_to_cpu(*(t_u16 *)
                                                          tlv->data));
                    break;
                case 4:
                    printf("%ld",
                           (unsigned long) uap_le32_to_cpu(*(t_u32 *)
                                                           tlv->data));
                    break;
                default:
                    for (j = 0; j < oid_size; j++) {
                        printf("%d ", (t_u8) tlv->data[i]);
                    }
                    break;
                }
                /** Next TLV */
...
check_sys_config(t_u8 * buf, t_u16 len)
{
    ...
    tlvbuf_protocol *protocol_tlv;
    tlvbuf_wep_key *wep_key_tlv;
    tlvbuf_wpa_passphrase *passphrase_tlv;
    tlvbuf_tx_data_rate *tx_data_rate_tlv;
    tlvbuf_mcbc_data_rate *mcbc_data_rate_tlv;
    t_u16 protocol = 0;
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header) && (ret != UAP_FAILURE)) {
        ...
        case MRVL_CHANNELCONFIG_TLV_ID:
            ...
            if (!(channel_tlv->band_config_type & BAND_CONFIG_ACS_MODE)) {
                if (state_80211d) {
                    if (check_channel_validity_11d(channel_tlv->chan_number, 1)
                        == UAP_FAILURE)
                        return UAP_FAILURE;
                }
            } else {
        ...
        case MRVL_CHANNELLIST_TLV_ID:
            ...
            for (i = 0; (unsigned int) i < chan_list_len; i++) {
    ...
    if (state_80211d && acs_mode_enabled) {
        ...
        for (i = 0; i < chan_list_len; i++) {
            if (check_channel_validity_11d(pchan_list->chan_number, 0) ==
                UAP_FAILURE)
                return UAP_FAILURE;
            pchan_list++;
        }
    }
    if (enable_40Mhz && !secondary_ch_set) {
...
print_sys_cfg_custom_ie_usage(void)
{
    ...
    printf("\n                -1 - Append/Delet IE automatically");
    printf
...
apcmd_sys_cfg_custom_ie(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        if ((atoi(argv[1]) < -1) || (atoi(argv[1]) > 3)) {
            printf
                ("ERR:Illegal index %s. Must be either '-1', '0', '1', '2', or '3'.\n",
                 argv[0]);
            print_sys_cfg_custom_ie_usage();
            return UAP_FAILURE;
        }
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header)) {
        ...
        case HT_CAPABILITY_TLV_ID:
            ...
            if (!ht_cap_tlv->ht_cap.supported_mcs_set[0]) {
                ...
                printf("ht_cap_info=0x%x, ampdu_param=0x%x",
                       uap_le16_to_cpu(ht_cap_tlv->ht_cap.ht_cap_info),
