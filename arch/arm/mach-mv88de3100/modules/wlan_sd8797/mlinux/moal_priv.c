...
woal_tx_bf_cfg_ioctl(moal_private * priv, struct iwreq *wrq)
{
    ...
    if (char_count) {
        ...
        if (char_count <= 2) {  /* GET */
            ...
            /* BF Action specific data */
            memset(peer, 0, MLAN_MAC_ADDR_LENGTH);
            switch (bf_action) {
            case TRIGGER_SOUNDING_FOR_PEER:
                /* First arg = 2 BfAction Second arg = 18 MAC
                   "00:50:43:20:BF:64;" Third arg = 1 Status */
                if (char_count < 21) {
                    PRINTM(MERROR, "Invalid argument\n");
                    ret = -EINVAL;
                    goto done;
                ...
                if (woal_atoi(&status, &buf[20]) != MLAN_STATUS_SUCCESS) {
                    ret = -EINVAL;
                    goto done;
                }
                bf_sound->status = status;
                break;
            case SET_GET_BF_PERIODICITY:
                /* First arg = 2 BfAction Second arg = 1 (min char) TX BF
                   interval 10 (max char) u32 maximum value 4294967295 */
                if (char_count < 3 || char_count > 12) {
                    PRINTM(MERROR, "Invalid argument\n");
                    ret = -EINVAL;
                    goto done;
                }
                if (woal_atoi(&interval, &buf[2]) != MLAN_STATUS_SUCCESS) {
                    ret = -EINVAL;
                    goto done;
                }
                bf_cfg.body.interval = interval;
                break;
            case NO_TX_BF_FOR_PEER:
                ...
                woal_mac2u8(bf_no_tx->peer_mac, &buf[2]);
                break;
            case SET_SNR_THR_PEER:
                /* First arg = 2 BfAction Second arg = 18 MAC
                   "00:50:43:20:BF:64;" Third arg = 1 SNR */
                if (char_count < 21) {
                    PRINTM(MERROR, "Invalid argument\n");
                    ret = -EINVAL;
                    goto done;
    ...
    if (action == MLAN_ACT_GET) {
        ...
        case TRIGGER_SOUNDING_FOR_PEER:
            ...
            break;
        case SET_GET_BF_PERIODICITY:
            data_length += sprintf(buf + data_length, "%ld",
                                   bf_cfg.body.interval);
            break;
        case NO_TX_BF_FOR_PEER:
