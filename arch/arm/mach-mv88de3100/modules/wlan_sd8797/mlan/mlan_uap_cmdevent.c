                       ...
                       IN t_u16 cmd_action, IN pmlan_ioctl_req pioctl_buf)
{
    ...
    if ((bss->param.bss_config.chan_list.num_of_chan) &&
        (bss->param.bss_config.chan_list.num_of_chan <= MLAN_MAX_CHANNEL)) {
                         ...
                         IN t_void * pdata_buf, IN t_void * pcmd_buf)
{
    ...
    case HostCmd_CMD_SET_BSS_MODE:
        ...
        if (pdata_buf)
            ...
            cmd_ptr->params.bss_mode.con_type = BSS_MODE_WFD_GO;
        cmd_ptr->size =
...
wlan_ops_uap_process_event(IN t_void * priv)
{
    ...
    case EVENT_MICRO_AP_STA_ASSOC:
        ...
        if (pmpriv->is_11n_enabled) {
            sta_ptr->is_11n_enabled =
                (t_u8) wlan_check_11n_capability(pmpriv, pmbuf);
