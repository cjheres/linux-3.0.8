                         ...
                         IN pmlan_buffer pmbuf, IN mlan_status status)
{
    ...
    if (pmbuf->buf_type == MLAN_BUF_TYPE_DATA) {
        PRINTM(MDATA, "wlan_write_data_complete: DATA %p\n", pmbuf);
                ...
                if (pmadapter->rx_pause_flag &&
                    ...
                    if (wlan_prepare_cmd
                        (pmadapter->priv[pmbuf->bss_index],
                         HOST_CMD_RX_DATA_PAUSE, HostCmd_ACT_GEN_SET, 0, MNULL,
                         MNULL) != MLAN_STATUS_SUCCESS) {
                        PRINTM(MERROR,
                               "Could not send Rx data pause command\n");
                    }
                }
            }
