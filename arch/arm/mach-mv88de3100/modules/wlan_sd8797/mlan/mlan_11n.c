                   ...
                   IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    txbfcfg->action = wlan_cpu_to_le16(cmd_action);

    txbfcfg->bf_action = wlan_cpu_to_le16((t_u16) txbf->bf_action);
    if (cmd_action == HostCmd_ACT_GEN_SET) {
        ...
        case TRIGGER_SOUNDING_FOR_PEER:
            ...
            txbfcfg->body.bf_sound_args.status = txbf->body.bf_sound[0].status;
            break;
        case SET_GET_BF_PERIODICITY:
            txbfcfg->body.interval = wlan_cpu_to_le32(txbf->body.interval);
            break;
        case NO_TX_BF_FOR_PEER:
            memcpy(pmadapter, txbfcfg->body.peer_mac,
                   txbf->body.bf_no_tx[0].peer_mac, MLAN_MAC_ADDR_LENGTH);
            break;
                   ...
                   IN HostCmd_DS_COMMAND * resp, IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        case TRIGGER_SOUNDING_FOR_PEER:
            ...
            break;
        case SET_GET_BF_PERIODICITY:
            txbf->body.interval = wlan_le32_to_cpu(txbfcfg->body.interval);
            break;
        case NO_TX_BF_FOR_PEER:
            txbf->no_of_peers = *(t_u8 *) & txbfcfg->body;
            memcpy(pmadapter, (t_u8 *) txbf->body.bf_no_tx,
                   (t_u8 *) & txbfcfg->body + sizeof(t_u8),
                   sizeof(mlan_no_tx_args) * txbf->no_of_peers);
            break;
        case SET_SNR_THR_PEER:
            txbf->no_of_peers = *(t_u8 *) & txbfcfg->body;
            memcpy(pmadapter, (t_u8 *) txbf->body.bf_snr,
                   (t_u8 *) & txbfcfg->body + sizeof(t_u8),
                   sizeof(mlan_snr_thr_args) * txbf->no_of_peers);
            break;
        default:
            LEAVE();
            return MLAN_STATUS_FAILURE;
