...
wifidirect_ioctl(t_u8 * cmd, t_u16 * size, t_u16 buf_size)
{
    ...
    *(t_u32 *) cmd = buf_size - BUF_HEADER_SIZE;
    ...
    /* Perform ioctl */
    errno = 0;
    if (ioctl(sockfd, WIFIDIRECT_HOSTCMD, &ifr)) {
        perror("");
        printf("ERR:WIFIDIRECT_HOSTCMD is not supported by %s\n", dev_name);
        close(sockfd);
        return FAILURE;
    }
                              ...
                              t_u16 * ie_len_wifidirect, t_u16 * ie_len_wps)
{
    ...
    t_u8 wifidirect_secondary_dev_count =
        0, wifidirect_total_secondary_dev_count = 0;
...
wifidirectcmd_status(int argc, char *argv[])
{
    ...
    switch (data) {
    case 0:
        printf("wifidirect status = stopped\n");
        break;
    case 1:
        printf("wifidirect status = started\n");
        break;
    case 2:
        printf("wifidirect status = wifidirect group owner\n");
        break;
    case 3:
        printf("wifidirect status = wifidirect client\n");
        break;
    case 4:
        printf("wifidirect status = Find phase started\n");
        break;
    case 5:
        printf("wifidirect status = Find phase stopped\n");
        break;
    default:
        printf("wifidirect status = %d\n", data);
        break;
    }
    return;
}
        ...
        if (!strncasecmp("0x", argv[7], 2)) {
            ...
            if (strlen(argv[7]) / 2 != WIFIDIRECT_PSK_LEN_MAX) {
                printf("ERR:Incorrect PSK length %d. It should be %d.\n",
                       strlen(argv[7]) / 2, WIFIDIRECT_PSK_LEN_MAX);
                goto done;
            }
