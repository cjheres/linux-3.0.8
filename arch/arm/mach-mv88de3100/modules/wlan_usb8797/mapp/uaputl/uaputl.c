    ...
    case TX_BF_FOR_PEER_ENBL:
        for (i = 0; i < param->no_of_peers; i++) {
            printf("PEER MAC = %02x:%02x:%02x:%02x:%02x:%02x\n",
                   ...
                   tx_bf_peer->peer_mac[4], tx_bf_peer->peer_mac[5]);
            printf("BF Status : %s\n",
                   tx_bf_peer->bf_enbl ? "ENABLED" : "DISABLED");
            printf("Sounding Status : %s\n",
                   tx_bf_peer->sounding_enbl ? "ENABLED" : "DISABLED");
            printf("FB Type : %d\n", tx_bf_peer->fb_type);
            tx_bf_peer++;
        }
        break;
...
apcmd_sys_cfg_tx_bf(int argc, char *argv[])
{
    ...
    case BF_GLOBAL_CONFIGURATION:
        ...
        if (argc == 1) {
            ...
            param.action = ACTION_SET;
            param.body.bf_global_cfg.bf_enbl = atoi(argv[BF_ENABLE_PARAM]);
            param.body.bf_global_cfg.sounding_enbl =
                atoi(argv[SOUND_ENABLE_PARAM]);
            param.body.bf_global_cfg.fb_type = atoi(argv[FB_TYPE_PARAM]);
            ...
            param.body.bf_global_cfg.bf_mode = atoi(argv[BF_MODE_PARAM]);
        }
        break;
    case TRIGGER_SOUNDING_FOR_PEER:
        if (argc != 2) {
    ...
    case SET_GET_BF_PERIODICITY:
        ...
        if ((ret =
             mac2raw(argv[1],
                     param.body.bf_periodicity[0].peer_mac)) != UAP_SUCCESS) {
            printf("ERR: %s Address\n",
                   ret == UAP_FAILURE ? "Invalid MAC" : ret ==
                   UAP_RET_MAC_BROADCAST ? "Broadcast" : "Multicast");
            return UAP_FAILURE;
    ...
    case TX_BF_FOR_PEER_ENBL:
        ...
        if (argc == 1) {
            ...
            if ((ret =
                 mac2raw(argv[1],
                         param.body.tx_bf_peer[0].peer_mac)) != UAP_SUCCESS) {
                printf("ERR: %s Address\n",
                       ret == UAP_FAILURE ? "Invalid MAC" : ret ==
                       UAP_RET_MAC_BROADCAST ? "Broadcast" : "Multicast");
                return UAP_FAILURE;
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header) && (ret != UAP_FAILURE)) {
        ...
        case MRVL_CHANNELCONFIG_TLV_ID:
            ...
            if ((!state_80211d) && (channel_tlv->chan_number <= MAX_CHANNELS_BG)
                ...
                printf("ERR: Invalid channel in 2.4GHz band, 802.11d %s",
                       (state_80211d) ? "enabled" : "disabled");
                printf(", country %c%c", country_80211d[0], country_80211d[1]);
                printf("!\n");
                return UAP_FAILURE;
            }
        ...
        case MRVL_CHANNELLIST_TLV_ID:
            ...
            for (i = 0; i < chan_list_len; i++) {
                ...
                if ((!state_80211d) &&
                    ...
                    printf
                        ("ERR: Invalid scan channel in 2.4GHz band, 802.11d %s",
                         (state_80211d) ? "enabled" : "disabled");
                    printf(",country %c%c", country_80211d[0],
                           country_80211d[1]);
                    printf("!\n");
                    return UAP_FAILURE;
                }
...
apcmd_mgmt_frame_control(int argc, char *argv[])
{
    ...
    if (argc) {
        param[0] = ACTION_SET;
        param[1] = (t_u16) A2HEXDECIMAL(argv[0]);
    } else {
        param[0] = ACTION_GET;
    }
    /* Open socket */
    ...
    /* Perform ioctl */
    errno = 0;
    if (ioctl(sockfd, UAP_MGMT_FRAME_CONTROL, &ifr)) {
        perror("");
        printf("ERR:UAP_MGMT_FRAME_CONTROL failed\n");
        close(sockfd);
        return UAP_FAILURE;
    }
    if (!argc) {
    ...
    case TXRATECFG:
        if (argc != 1) {
            ...
            if (IS_HEX_OR_DIGIT(argv[0]) == UAP_FAILURE) {
                ...
                if ((A2HEXDECIMAL(argv[0]) != 0) && A2HEXDECIMAL(argv[0]) > 27
...
uap_ioctl_reroute(t_u8 * cmd, t_u16 * size, t_u16 buf_size, int *reroute_ret)
{
    ...
    case HostCmd_CMD_802_11D_DOMAIN_INFO:
        ...
        tlv = (tlvbuf_header *) (cmd + sizeof(apcmdbuf_cfg_80211d));
