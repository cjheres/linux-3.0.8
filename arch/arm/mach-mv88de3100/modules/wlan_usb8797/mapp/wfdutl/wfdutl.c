/** @file  wfdutl.c
 *
 *  @brief Program to configure wfd parameters.
 *
 *   Usage: ./wfdutl <iface> <command> <params>
 *
...
/**
 *  @brief Prints usage information of wfdutl
 *
 *  @return          N/A
 */
static void
print_tool_usage(void)
{
    ...
    printf("Usage:\n");
    printf("./wfdutl <iface> wfd_mode [mode]\n"
           "./wfdutl <iface> wfd_config [*.conf]\n"
           "./wfdutl <iface> wfd_params_config [*.conf]\n"
           "./wfdutl <iface> wfd_action_frame <*.conf>|"
           "	<PeerAddr> <Category> <OuiSubtype> <DialogToken>\n"
           "./wfdutl <iface> wfd_discovery_request <*.conf>\n"
           "./wfdutl <iface> wfd_discovery_response <*.conf>\n"
           "./wfdutl <iface> wfd_gas_comeback_request <*.conf>\n"
           "./wfdutl <iface> wfd_gas_comeback_response <*.conf>\n");
    printf("\nPlease see example configuration file config/wfd.conf\n\n");
    ...
    printf
        ("./wfdutl <iface> wfd_cfg_discovery_period [<MinDiscPeriod> <MaxDiscPeriod>]\n"
         ...
         "./wfdutl <iface> wfd_cfg_noa <enable | disable> [<counttype> <duration> <interval>]\n"
         "./wfdutl <iface> wfd_cfg_opp_ps [<enable> <CTWindow>]\n"
         "./wfdutl <iface> wfd_cfg_invitation_list [mac_address]\n"
         "./wfdutl <iface> wfd_cfg_listen_channel [listenChannel]\n"
         "./wfdutl <iface> wfd_cfg_op_channel [operatingChannel]\n"
         "./wfdtul <iface> wfd_cfg_persistent_group_record [index] [role]\n"
         "	[<groupbss> <deviceId> <ssid> <psk>] [peermac1] [peermac2]\n"
         "./wfdtul <iface> wfd_cfg_persistent_group_invoke [index] | <cancel>\n"
 ...
 */
static int
wfd_ioctl(t_u8 * cmd, t_u16 * size, t_u16 buf_size)
{
    struct ifreq ifr;
    ...
    *(t_u32 *) cmd = buf_size - BUF_HEADER_SIZE;
    ...
    if (header->cmd_code == HostCmd_CMD_WFD_PARAMS_CONFIG) {
        wfd_params_config *param_cfg;
        param_cfg = (wfd_params_config *) cmd;
    ...
    /* Perform ioctl */
    errno = 0;
    if (ioctl(sockfd, WFD_HOSTCMD, &ifr)) {
        perror("");
        printf("ERR:WFD_HOSTCMD is not supported by %s\n", dev_name);
        close(sockfd);
        return FAILURE;
    }
    endian_convert_response_header(header);
    header->cmd_code &= HostCmd_CMD_ID_MASK;
    header->cmd_code |= WFDCMD_RESP_CHECK;
    *size = header->size;
...
/**
 *  @brief Show usage information for the wfd_gas_comeback_discovery commands
 *
 *  $return         N/A
 */
static void
print_wfd_gas_comeback_usage(void)
{
    printf("\nUsage : wfd_gas_comeback_request/response [CONFIG_FILE]\n");
    printf("CONFIG_FILE contains WFD GAS comeback request/response payload.\n");
    return;
}

/**
 *  @brief Creates a wfd_gas_comeback_service_discovery request/response and
 *         sends to the driver
 *
 *  Usage: "Usage : wfd_gas_comeback_request/response [CONFIG_FILE]"
 *
 ...
 **/
static void
wfdcmd_gas_comeback_discovery(int argc, char *argv[])
{
    wfd_gas_comeback_request *req_buf = NULL;
    wfd_gas_comeback_response *resp_buf = NULL;
    char *line = NULL;
    FILE *config_file = NULL;
    int i, opt, li = 0, arg_num = 0, ret = 0, wfd_level = 0;
    char *args[30], *pos = NULL, wfd_mac[20], wfd_cmd[40];
    ...
    strncpy(wfd_cmd, argv[2], sizeof(wfd_cmd));
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_gas_comeback_usage();
            return;
        }
    }
    argc -= optind;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (!cmd_found && strncmp(args[0], wfd_cmd, strlen(args[0])))
            continue;
        cmd_found = 1;

        if (strcmp(args[0], "wfd_gas_comeback_request") == 0) {
            wfd_level = WFD_DISCOVERY_REQUEST_RESPONSE;
            /* For wfd_service_gas_comeback, basic initialization here */
            cmd_len = sizeof(wfd_gas_comeback_request);
            buffer = (t_u8 *) malloc(cmd_len);
            if (!buffer) {
                printf("ERR:Cannot allocate memory!\n");
                goto done;
            }
            req_buf = (wfd_gas_comeback_request *) buffer;
            req_buf->cmd_code = HostCmd_CMD_WFD_SERVICE_DISCOVERY;
            req_buf->size = cmd_len;
            req_buf->seq_num = 0;
            req_buf->result = 0;

        } else if (strcmp(args[0], "wfd_gas_comeback_response") == 0) {
            wfd_level = WFD_DISCOVERY_REQUEST_RESPONSE;
            req_resp = 1;
            /* For wfd_service_discovery, basic initialization here */
            cmd_len = sizeof(wfd_gas_comeback_response);
            buffer = (t_u8 *) malloc(cmd_len);
            if (!buffer) {
                printf("ERR:Cannot allocate memory!\n");
                goto done;
            }
            resp_buf = (wfd_gas_comeback_response *) buffer;
            resp_buf->cmd_code = HostCmd_CMD_WFD_SERVICE_DISCOVERY;
            ...
            if (is_input_valid(WFD_CATEGORY, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            ...
            if (is_input_valid(WFD_ACTION, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_DIALOGTOKEN, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_GAS_COMEBACK_DELAY, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_DISC_ADPROTOIE, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_DISC_INFOID, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            for (i = 0; i < arg_num - 1; i++)
            ...
            if (is_input_valid(WFD_OUI, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            for (i = 0; i < arg_num - 1; i++)
            ...
            if (is_input_valid(WFD_OUISUBTYPE, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            wfd_level = WFD_DISCOVERY_QUERY;
        } else if (strcmp(args[0], "RequestLen") == 0 ||
                   strcmp(args[0], "ResponseLen") == 0) {
            wfd_level = WFD_DISCOVERY_SERVICE;
            query_len += 2;
        } else if (strcmp(args[0], "VendorLen") == 0) {
            wfd_level = WFD_DISCOVERY_VENDOR;
            service_len += 2;
            query_len += 2;
        } else if (strcmp(args[0], "QueryData") == 0 ||
                   strcmp(args[0], "ResponseData") == 0) {
            wfd_level = WFD_DISCOVERY_QUERY_RESPONSE_PER_PROTOCOL;
        } else if (strcmp(args[0], "ServiceProtocol") == 0) {
            if (is_input_valid(WFD_DISC_SERVICEPROTO, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid
                (WFD_SERVICEUPDATE_INDICATOR, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_DISC_SERVICETRANSACID, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_DISC_SERVICE_STATUS, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (args[1][0] == '"') {
                ...
                buffer = realloc(buffer, cmd_len + dns_len);
                if (!buffer) {
            ...
            if (is_input_valid(WFD_DISC_DNSTYPE, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            *(&resp_buf->disc_resp.u.bonjour.dns_type + dns_len) =
            ...
            if (is_input_valid(WFD_DISC_BONJOUR_VERSION, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            *(&resp_buf->disc_resp.u.bonjour.version + dns_len) =
            ...
            if (is_input_valid(WFD_DISC_UPNP_VERSION, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (args[1][0] == '"') {
                ...
                buffer = realloc(buffer, cmd_len + upnp_len);
                if (!buffer) {
            ...
            if (args[1][0] == '"') {
                ...
                buffer = realloc(buffer, cmd_len + record_len);
                if (!buffer || !req_resp) {
            ...
            if (wfd_level) {
                if (wfd_level == WFD_DISCOVERY_REQUEST_RESPONSE)
                    break;
                wfd_level--;
            }
        }
    ...
    wfd_ioctl((t_u8 *) buffer, &cmd_len, cmd_len);
  done:
    fclose(config_file);
    if (buffer)
        free(buffer);
...
/**
 *  @brief Show usage information for the wfd_discovery commands
 *
 *  $return         N/A
 */
static void
print_wfd_discovery_usage(void)
{
    printf("\nUsage : wfd_discovery_request/response [CONFIG_FILE]\n");
    printf("CONFIG_FILE contains WFD service discovery payload.\n");
    return;
}

/**
 *  @brief Creates a wfd_service_discovery request/response and
 *         sends to the driver
 *
 *  Usage: "Usage : wfd_discovery_request/response [CONFIG_FILE]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         SUCCESS or FAILURE
 **/
static void
wfdcmd_service_discovery(int argc, char *argv[])
{
    wfd_discovery_request *req_buf = NULL;
    wfd_discovery_response *resp_buf = NULL;
    char *line = NULL;
    FILE *config_file = NULL;
    int i, opt, li = 0, arg_num = 0, ret = 0, wfd_level = 0;
    char *args[30], *pos = NULL, wfd_mac[20], wfd_cmd[32];
    t_u8 dev_address[ETH_ALEN], cmd_found = 0;
    t_u8 *buffer = NULL;
    t_u8 req_resp = 0;          /* req = 0, resp = 1 */
    t_u16 cmd_len = 0, query_len = 0, vendor_len = 0, service_len = 0;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "wfd_discovery_request") == 0) {
            wfd_level = WFD_DISCOVERY_REQUEST_RESPONSE;
            /* For wfd_service_discovery, basic initialization here */
            cmd_len = sizeof(wfd_discovery_request);
            buffer = (t_u8 *) malloc(cmd_len);
            if (!buffer) {
                printf("ERR:Cannot allocate memory!\n");
                goto done;
            }
            req_buf = (wfd_discovery_request *) buffer;
            req_buf->cmd_code = HostCmd_CMD_WFD_SERVICE_DISCOVERY;
            req_buf->size = cmd_len;
            req_buf->seq_num = 0;
            req_buf->result = 0;

        } else if (strcmp(args[0], "wfd_discovery_response") == 0) {
            wfd_level = WFD_DISCOVERY_REQUEST_RESPONSE;
            req_resp = 1;
            /* For wfd_service_discovery, basic initialization here */
            cmd_len = sizeof(wfd_discovery_response);
            buffer = (t_u8 *) malloc(cmd_len);
            if (!buffer) {
                printf("ERR:Cannot allocate memory!\n");
                goto done;
            }
            resp_buf = (wfd_discovery_response *) buffer;
            resp_buf->cmd_code = HostCmd_CMD_WFD_SERVICE_DISCOVERY;
            resp_buf->size = cmd_len;
            resp_buf->seq_num = 0;
            resp_buf->result = 0;
...
/**
 *  @brief Show usage information for the wfd_config command
 *
 *  $return         N/A
 */
static void
print_wfd_config_usage(void)
{
    printf("\nUsage : wfd_config [CONFIG_FILE]\n");
    printf
        ("\nIf CONFIG_FILE is provided, a 'set' is performed, else a 'get' is performed.\n");
    printf("CONFIG_FILE contains all WFD parameters.\n");
    return;
}
 ...
 *  @param ie_len_wfd Length of wfd parameters to return
 *  @param ie_len_wps Length of WPS parameters to return
 *  @return          SUCCESS or FAILURE
 */
static void
wfd_file_params_config(char *file_name, char *cmd_name, t_u8 * pbuf,
                       t_u16 * ie_len_wfd, t_u16 * ie_len_wps)
{
    FILE *config_file = NULL;
    char *line = NULL;
    int i, li = 0, arg_num = 0, ret = 0, wfd_level = 0, no_of_chan_entries =
        0, no_of_noa = 0;
    int secondary_index = -1, flag = 1, group_secondary_index = -1;
    char **args = NULL;
    char *pos = NULL;
    char wfd_mac[20], country[4], wfd_ssid[33];
    ...
    t_u8 *extra = NULL;
    t_u8 *buffer = pbuf;
    t_u16 cmd_len_wfd = 0, cmd_len_wps = 0, tlv_len = 0, extra_len, temp;
    t_u16 wps_model_len = 0, wps_serial_len = 0;
    t_u16 pri_category, pri_sub_category, config_methods;
    t_u16 sec_category, sec_sub_category, group_sec_sub_category =
        0, group_sec_category = 0;
    t_u16 avail_period, avail_interval;
    t_u8 secondary_oui[4], group_secondary_oui[4];
    t_u16 WPS_specconfigmethods, WPS_associationstate,
        WPS_configurationerror, WPS_devicepassword;
    t_u8 dev_capability = 0, group_capability = 0, cmd_found = 0,
        group_owner_intent, primary_oui[4], iface_count,
        regulatory_class = 0, channel_number = 0, manageability = 0,
        op_regulatory_class = 0, op_channel_number = 0, invitation_flag = 0;
    t_u8 WPS_version, WPS_setupstate, WPS_requesttype, WPS_responsetype,
        WPS_UUID[WPS_UUID_MAX_LEN],
        WPS_primarydevicetype[WPS_DEVICE_TYPE_MAX_LEN], WPS_RFband,
        WPS_modelnumber[4], WPS_serialnumber[4];
    t_u8 go_config_timeout, client_config_timeout;
    ...
    t_u8 wfd_client_dev_count = 0, wfd_client_dev_index = 0, temp8;
    wfd_client_dev_info wfd_client_dev_info_list[MAX_SECONDARY_DEVICE_COUNT];
    t_u8 wfd_secondary_dev_count = 0, wfd_total_secondary_dev_count = 0;
    t_u8 wfd_group_total_ssid_len = 0, tlv_offset = 0, temp_dev_size = 0;
    t_u8 noa_index = 0, opp_ps = 0, ctwindow_opp_ps = 0, count_type;
    t_u32 duration, interval, start_time;
    t_u16 total_chan_len = 0;
    t_u8 chan_entry_regulatory_class, chan_entry_num_of_channels;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "wfd_config") == 0) {
        ...
        } else if (strncmp(args[0], "GroupWfdDeviceAddress", 21) == 0) {
            strncpy(wfd_mac, args[1], 20);
            if ((ret = mac2raw(wfd_mac, group_dev_address)) != SUCCESS) {
                printf("ERR: %s Address \n",
                       ret == FAILURE ? "Invalid MAC" : ret ==
                       WFD_RET_MAC_BROADCAST ? "Broadcast" : "Multicast");
                goto done;
            }
            wfd_client_dev_index++;
            if (wfd_client_dev_index > wfd_client_dev_count) {
                printf
                    ("ERR: No of Client Dev count is less than no of client dev configs!!\n");
                goto done;
            }
            group_secondary_index = 0;
            wfd_secondary_dev_count = 0;
            tlv_offset =
                wfd_group_total_ssid_len +
                wfd_total_secondary_dev_count * WPS_DEVICE_TYPE_LEN;
            memcpy(wfd_client_dev_info_list[wfd_client_dev_index - 1].
                   wfd_dev_address + tlv_offset, group_dev_address, ETH_ALEN);
        } else if (strncmp(args[0], "GroupWfdIntfAddress", 19) == 0) {
            strncpy(wfd_mac, args[1], 20);
            if ((ret = mac2raw(wfd_mac, group_dev_address)) != SUCCESS) {
                printf("ERR: %s Address \n",
                       ret == FAILURE ? "Invalid MAC" : ret ==
            ...
            memcpy(wfd_client_dev_info_list[wfd_client_dev_index - 1].
                   wfd_intf_address + tlv_offset, group_dev_address, ETH_ALEN);
        } else if (strncmp(args[0], "GroupWfdDeviceCapab", 19) == 0) {
            if (is_input_valid(WFD_DEVICECAPABILITY, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            temp8 = (t_u8) atoi(args[1]);
            memcpy(&wfd_client_dev_info_list[wfd_client_dev_index - 1].
                   wfd_dev_capability + tlv_offset, &temp8, sizeof(temp8));
        } else if (strncmp(args[0], "GroupWfdWPSConfigMethods", 24) == 0) {
            if (is_input_valid(WFD_WPSCONFMETHODS, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_PRIDEVTYPECATEGORY, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_PRIDEVTYPEOUI, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            for (i = 0; i < 4; i++) {
                ...
                memcpy(&wfd_client_dev_info_list[wfd_client_dev_index - 1].
                   ...
                   0) {
            if (is_input_valid(WFD_PRIDEVTYPESUBCATEGORY, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid
                (WFD_GROUP_SECONDARYDEVCOUNT, arg_num - 1,
                 args + 1) != SUCCESS) {
                goto done;
            }
            group_secondary_dev_count = (t_u8) atoi(args[1]);
            memcpy(&wfd_client_dev_info_list[wfd_client_dev_index - 1].
                   wfd_secondary_dev_count + tlv_offset,
                   &group_secondary_dev_count,
                   sizeof(group_secondary_dev_count));
            wfd_total_secondary_dev_count += group_secondary_dev_count;
        ...
        } else if (strncmp(args[0], "GroupWfdDeviceCount", 19) == 0) {
            if (is_input_valid(WFD_SECONDARYDEVCOUNT, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            wfd_client_dev_count = (t_u8) atoi(args[1]);
        } else if (strncmp(args[0], "GroupWfdDeviceName", 18) == 0) {

            if (is_input_valid(WFD_GROUP_WFD_DEVICE_NAME, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            memcpy(((t_u8 *) &
                    wfd_client_dev_info_list[wfd_client_dev_index -
                                             1].wfd_device_name_type +
                    tlv_offset), &temp, sizeof(temp));
            temp = htons(strlen(wifi_group_direct_ssid));
            memcpy(((t_u8 *) &
                    wfd_client_dev_info_list[wfd_client_dev_index -
                                             1].wfd_device_name_len +
                    tlv_offset), &temp, sizeof(temp));
            memset(((t_u8 *) &
                    wfd_client_dev_info_list[wfd_client_dev_index -
                                             1].wfd_device_name + tlv_offset),
                   0, strlen(wifi_group_direct_ssid));
            memcpy(((t_u8 *) &
                    wfd_client_dev_info_list[wfd_client_dev_index -
                                             1].wfd_device_name + tlv_offset),
                   &wifi_group_direct_ssid, strlen(wifi_group_direct_ssid));
            wfd_group_total_ssid_len += strlen(wifi_group_direct_ssid);

            if (wfd_client_dev_index - 1) {
                temp_dev_size =
                    sizeof(wfd_client_dev_info) +
                ...
                temp_dev_size =
                    ...
                    strlen(wifi_group_direct_ssid) +
                    group_secondary_dev_count * WPS_DEVICE_TYPE_LEN;
                wfd_client_dev_info_list[wfd_client_dev_index - 1].dev_length =
                    sizeof(wfd_client_dev_info) +
                    strlen(wifi_group_direct_ssid) +
                    group_secondary_dev_count * WPS_DEVICE_TYPE_LEN;
            }
            ...
            if (is_input_valid(WFD_GROUPCAPABILITY, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            /* Intent -> 0 - 15 */
            if (is_input_valid(WFD_INTENT, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            group_owner_intent = (t_u8) atoi(args[1]);
        } else if (strcmp(args[0], "RegulatoryClass") == 0) {
            if (is_input_valid(WFD_REGULATORYCLASS, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_MANAGEABILITY, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            manageability = (t_u8) atoi(args[1]);
        } else if (strcmp(args[0], "InvitationFlag") == 0) {
            if (is_input_valid(WFD_INVITATIONFLAG, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            invitation_flag = (t_u8) atoi(args[1]);
        } else if (strcmp(args[0], "CountryString") == 0) {
            if (is_input_valid(WFD_COUNTRY, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_NO_OF_CHANNELS, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_NOA_INDEX, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            noa_index = (t_u8) atoi(args[1]);
        } else if (strcmp(args[0], "OppPS") == 0) {
            if (is_input_valid(WFD_OPP_PS, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            opp_ps = (t_u8) atoi(args[1]);
        } else if (strcmp(args[0], "CTWindow") == 0) {
            if (is_input_valid(WFD_CTWINDOW, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_COUNT_TYPE, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            no_of_noa++;
            ...
            if (is_input_valid(WFD_DURATION, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_INTERVAL, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_START_TIME, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_INTERFACECOUNT, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_ATTR_CONFIG_TIMEOUT, arg_num - 1, args + 1)
                != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_ATTR_EXTENDED_TIME, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            memset(wfd_ssid, 0, sizeof(wfd_ssid));
            strncpy(wfd_ssid, args[1], sizeof(wfd_ssid));
        } else if (strcmp(args[0], "WPSVersion") == 0) {
            if (is_input_valid(WFD_WPSVERSION, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            WPS_version = (t_u8) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "WPSSetupState") == 0) {
            if (is_input_valid(WFD_WPSSETUPSTATE, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            WPS_setupstate = (t_u8) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "WPSDeviceName") == 0) {
            if (is_input_valid(WFD_WPSDEVICENAME, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_WPSREQRESPTYPE, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_WPSSPECCONFMETHODS, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            WPS_specconfigmethods = (t_u16) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "WPSUUID") == 0) {
            if (is_input_valid(WFD_WPSUUID, arg_num - 1, args + 1) != SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_WPSMODELNAME, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            ...
            if (is_input_valid(WFD_WPSMODELNUMBER, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            for (i = 0; i < arg_num - 1; i++)
            ...
            if (is_input_valid(WFD_WPSSERIALNUMBER, arg_num - 1, args + 1) !=
                SUCCESS) {
                goto done;
            }
            for (i = 0; i < arg_num - 1; i++)
            ...
            case WFD_GROUP_OWNER_INTENT_CONFIG:
                {
            ...
            case WFD_INVITATION_FLAG_CONFIG:
                {
            ...
            case WFD_NOA_DESCRIPTOR:
                {
                    wfd_level = WFD_NOTICE_OF_ABSENCE;
                    flag = 0;
                    break;
                }
            case WFD_CHANNEL_CONFIG:
                {
            ...
            case WFD_DEVICE_INFO_CONFIG:
                {
                    ...
                    memcpy(((t_u8 *) (&tlv->device_name_type)) +
                           ...
                           sizeof(temp));
                    temp = htons(strlen(wfd_ssid));
                    memcpy(((t_u8 *) (&tlv->device_name_len)) +
                           secondary_dev_count * WPS_DEVICE_TYPE_LEN, &temp,
                           sizeof(temp));
                    memcpy(((t_u8 *) (&tlv->device_name)) +
                           secondary_dev_count * WPS_DEVICE_TYPE_LEN, wfd_ssid,
                           strlen(wfd_ssid));
                    flag = 1;
                    break;
                }
            case WFD_GROUP_INFO_CONFIG:
                {
                    tlvbuf_wfd_group_info *tlv = NULL;
                    /* Append a new TLV */
                    tlv_offset = wfd_group_total_ssid_len
                        + wfd_total_secondary_dev_count * WPS_DEVICE_TYPE_LEN;
                    tlv_len = sizeof(tlvbuf_wfd_group_info)
                        + wfd_client_dev_count * sizeof(wfd_client_dev_info) +
                        tlv_offset;
                    tlv = (tlvbuf_wfd_group_info *) (buffer + cmd_len_wfd);
                    cmd_len_wfd += tlv_len;
                    /* Set TLV fields */
                    tlv->tag = TLV_TYPE_WFD_GROUP_INFO;
                    tlv->length = tlv_len;
                    memcpy(tlv->wfd_client_dev_list, wfd_client_dev_info_list,
                           wfd_client_dev_count * sizeof(wfd_client_dev_info) +
                           tlv_offset);
                    /* Parameters within secondary_dev_info are already
                       htons'ed */
            ...
            case WFD_GROUP_SEC_INFO_CONFIG:
                {
                    wfd_level = WFD_GROUP_CLIENT_INFO_CONFIG;

                    if (wfd_client_dev_index && group_secondary_index) {
                        memset(((t_u8 *) &
                                wfd_client_dev_info_list[wfd_client_dev_index -
                                                         1].
                                wfd_secondary_dev_info + tlv_offset), 0,
                               group_secondary_index * WPS_DEVICE_TYPE_LEN);
                        memcpy(((t_u8 *) &
                                wfd_client_dev_info_list[wfd_client_dev_index -
                                                         1].
                                wfd_secondary_dev_info + tlv_offset),
            ...
            case WFD_GROUP_CLIENT_INFO_CONFIG:
                {
                    wfd_level = WFD_GROUP_INFO_CONFIG;
                    flag = 0;
                    break;
                }
            case WFD_DEVICE_SEC_INFO_CONFIG:
                {
                    wfd_level = WFD_DEVICE_INFO_CONFIG;
                    flag = 0;
                    break;
                }
            case WFD_GROUP_ID_CONFIG:
                {
                    tlvbuf_wfd_group_id *tlv = NULL;
                    /* Append a new TLV */
                    tlv_len = sizeof(tlvbuf_wfd_group_id) + strlen(wfd_ssid);
                    ...
                    memcpy(tlv->group_ssid, wfd_ssid, strlen(wfd_ssid));
                    endian_convert_tlv_wfd_header_out(tlv);
                    break;
                }
            case WFD_GROUP_BSS_ID_CONFIG:
                {
...
#ifdef DEBUG
                    /* Debug print */
                    hexdump(buffer + sizeof(custom_ie) + cmd_len_wfd,
                            cmd_len_wps, ' ');
#endif
                    break;
                }
 ...
 *  @param data_len_wfd  Length of WFD data, 0 to get, else set.
 ...
 */
static int
wfd_ie_config(t_u16 * ie_index, t_u16 data_len_wfd,
...
#define MGMT_MASK_AUTO	0xffff
    ...
    if (!data_len_wfd) {
        ...
        *ie_index = 0xFFFF;
        ...
        if (*ie_index == 0xFFFF) {
            printf("No free IE buffer available\n");
            ret = FAILURE;
        }
    }
  _exit_:
 ...
 */
static void
wfdcmd_config(int argc, char *argv[])
{
    t_u8 *buf = NULL, *ptr, *dev_ptr, *array_ptr;
    t_u16 ie_len_wfd = 0, ie_len_wps = 0, ie_len;
    t_u16 ie_index = 0, temp;
    int i, opt, ret = SUCCESS;
    tlvbuf_custom_ie *tlv = NULL;
    custom_ie *ie_ptr = NULL;
    t_u16 len = 0, len_wfd = 0;
    t_u8 type = 0;
    ...
    if (argc >= 3) {
        ...
        wfd_file_params_config(argv[2], argv[1], buf
        ...
        ret = wfd_ie_config(&ie_index, 0, 0, buf);
        /* Print response */
        if (ret == SUCCESS && ie_index < 3) {
                    ...
                    case TLV_TYPE_WFD_CHANNEL_LIST:
                        {
                            ...
                            printf("\n");
                            temp_ptr =
                                (chan_entry *) wfd_tlv->wfd_chan_entry_list;
                            len_wfd =
                                le16_to_cpu(wfd_tlv->length) -
                    ...
                    case TLV_TYPE_WFD_NOTICE_OF_ABSENCE:
                        {
                            ...
                            temp_ptr =
                                (noa_descriptor *) wfd_tlv->
                                wfd_noa_descriptor_list;
                            len_wfd =
                                le16_to_cpu(wfd_tlv->length) -
                    ...
                    case TLV_TYPE_WFD_GROUP_INFO:
                        {
                            ...
                            t_u8 no_of_wfd_clients = 0, wfd_client_dev_length =
                                0;
                            wfd_client_dev_info *temp_ptr;
                            temp_ptr =
                                (wfd_client_dev_info *) wfd_tlv->
                                wfd_client_dev_list;
                            ...
                            while (temp) {
                                ...
                                printf("\n");
                                printf
                                    ("\t Group Wfd Client Interface address - ");
                                print_mac(temp_ptr->wfd_intf_address);
                                printf("\n");
                                printf
                                    ("\t Group Wfd Client Device capability = %d\n",
                                     (int) temp_ptr->wfd_dev_capability);
                                printf
                                    ("\t Group Wfd Client Config methods - 0x%02X\n",
                                ...
                                for (i = 0;
                                     i < temp_ptr->wfd_secondary_dev_count;
                                     i++) {
                                    memcpy(&temp, array_ptr, sizeof(t_u16));
                                    printf
                            ...
                            printf("\n");
                            printf("\t Group Wfd Client Devices count = %d\n",
                                   no_of_wfd_clients);
                        }
                        break;
                    case TLV_TYPE_WFD_GROUP_ID:
                        {
                            tlvbuf_wfd_group_id *wfd_tlv =
                                (tlvbuf_wfd_group_id *) ptr;
                            printf("\t Group address - ");
                            print_mac(wfd_tlv->group_address);
                            printf("\n");
                            array_ptr = wfd_tlv->group_ssid;
                            printf("\t Group ssid =  ");
                            for (i = 0;
                                 (unsigned int) i < le16_to_cpu(wfd_tlv->length)
            ...
            printf("ERR:Could not retrieve wfd parameters\n");
        }
    }
...
/**
 *  @brief Show usage information for the wfd_params_config command
 *
 *  $return         N/A
 */
static void
print_wfd_params_config_usage(void)
{
    printf("\nUsage : wfd_params_config <CONFIG_FILE>\n");
 ...
 */
static void
wfdcmd_params_config(int argc, char *argv[])
{
    wfd_params_config *param_buf = NULL;
    tlvbuf_wps_ie *new_tlv = NULL;
    char *line = NULL, wfd_mac[20];
    FILE *config_file = NULL;
    ...
    param_buf->cmd_code = HostCmd_CMD_WFD_PARAMS_CONFIG;
    param_buf->seq_num = 0;
    param_buf->result = 0;
    ...
    if (argc >= 3) {
        ...
        while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
            ...
            if (strcmp(args[0], "EnableScan") == 0) {
                ...
                if (is_input_valid
                    (WFD_WPSPRIMARYDEVICETYPE, arg_num - 1, args + 1)
                    != SUCCESS) {
                    goto done;
                }
                for (i = 0; i < WPS_DEVICE_TYPE_MAX_LEN; i++)
    ...
    /* Send collective command */
    wfd_ioctl((t_u8 *) buffer, &cmd_len, MRVDRV_SIZE_OF_CMD_BUFFER);

    /* Process response */
    if (argc < 3) {
        /* Verify response */
        if (param_buf->cmd_code !=
            (HostCmd_CMD_WFD_PARAMS_CONFIG | WFDCMD_RESP_CHECK)) {
        ...
        if (param_buf->result == CMD_SUCCESS) {
            param_buf = (wfd_params_config *) buffer;
            new_tlv = (tlvbuf_wps_ie *) param_buf->wfd_tlv;
            tlv_len =
                param_buf->size - (sizeof(wfd_params_config) - BUF_HEADER_SIZE);
            if (tlv_len > sizeof(tlvbuf_wps_ie))
                printf("Wfd Parameter settings: \n");
            else
                printf("Wfd Parameter settings are not received.\n");
            while (tlv_len > sizeof(tlvbuf_wps_ie)) {
                endian_convert_tlv_header_in(new_tlv);
                switch (new_tlv->tag) {
                case MRVL_WFD_SCAN_ENABLE_TLV_ID:
                    {
                    ...
                    break;
                case MRVL_WFD_PEER_DEVICE_TLV_ID:
                    {
                    ...
                    break;
                case MRVL_WFD_DISC_PERIOD_TLV_ID:
                    {
                        t_u16 temp;
                    ...
                    break;
                case MRVL_WFD_SCAN_REQ_DEVICE_TLV_ID:
                    {
                    ...
                    break;
                case MRVL_WFD_DEVICE_STATE_TLV_ID:
                    {
...
/**
 *  @brief Show usage information for the wfd_action_frame command
 *
 *  $return         N/A
 */
static void
print_wfd_action_frame_usage(void)
{
    printf("\nUsage : wfd_action_frame <CONFIG_FILE> |"
           " [<PeerAddr> <Category> <OUISubtype> <DialogToken>]\n");
 ...
 *
 *  Usage: "Usage : wfd_action_frame <CONFIG_FILE> |
 *                  [<PeerAddr> <Category> <OUISubtype> <DialogToken>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 ...
 */
static void
wfdcmd_action_frame(int argc, char *argv[])
{
    ...
    if (config_file && argc == 3) {
        ...
        while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
            ...
            if (strcmp(args[0], "}") == 0) {
                ...
                if (is_input_valid(WFD_OUITYPE, arg_num - 1, args + 1)
                    != SUCCESS) {
                    goto done;
                }
        ...
        wfd_file_params_config(argv[2], argv[1], action_buf->ie_list,
                               &ie_len, NULL);
        if (ie_len >= MRVDRV_SIZE_OF_CMD_BUFFER) {
            printf("ERR:Too much data in configuration file %s.\n", argv[2]);
            free(buffer);
        ...
        if (is_input_valid(WFD_CATEGORY, 1, &argv[3])
            != SUCCESS) {
            goto done;
        }
        action_buf->category = (t_u8) atoi(argv[3]);

        if (is_input_valid(WFD_OUISUBTYPE, 1, &argv[4])
            != SUCCESS) {
            goto done;
        }
        action_buf->oui_sub_type = (t_u8) atoi(argv[4]);

        if (is_input_valid(WFD_DIALOGTOKEN, 1, &argv[5])
            != SUCCESS) {
            goto done;
        }
        ...
        action_buf->oui_type = OUI_TYPE_WFA_WFD;
    }
...
/**
 *  @brief Show usage information for the wfd status command
 *
 *  $return         N/A
 */
static void
print_wfd_status_usage(void)
{
    printf("\nUsage : wfd_mode [STATUS]");
    printf("\nOptions: STATUS :     0 - stop wfd status");
    printf("\n                      1 - start wfd status");
    printf("\n                      2 - start wfd group owner mode");
    printf("\n                      3 - start wfd client mode");
    printf("\n                      4 - start wfd find phase");
    printf("\n                      5 - stop  wfd find phase");
    printf("\n                  empty - get current wfd status\n");
    return;
}

/**
 *  @brief Creates wfd mode start or stop request and send to driver
 *
 *   Usage: "Usage : wfd_mode [STATUS]"
 *
 ...
 */
static void
wfdcmd_status(int argc, char *argv[])
{
    int opt, ret;
    t_u16 data;
    t_u16 cmd_len = 0;
    ...
    if (argc == 3) {
        if ((ISDIGIT(argv[2]) == 0) || (atoi(argv[2]) < 0) ||
            (atoi(argv[2]) >= 0xFF)) {
            printf
                ("ERR:Illegal wfd mode %s. Must be in range from '0' to '5'.\n",
                 argv[2]);
            print_wfd_status_usage();
            return;
        }
    ...
    cmd_buf->cmd_code = HostCmd_CMD_WFD_MODE_CONFIG;
    ...
    ret = wfd_ioctl((t_u8 *) buffer, &cmd_len, MRVDRV_SIZE_OF_CMD_BUFFER);
    if (ret != SUCCESS) {
        printf("Error executing wfd mode command\n");
        return;
    }
    ...
    switch (data) {
    case 0:
        printf("wfd status = stopped\n");
        break;
    case 1:
        printf("wfd status = started\n");
        break;
    case 2:
        printf("wfd status = wfd group owner\n");
        break;
    case 3:
        printf("wfd status = wfd client\n");
        break;
    case 4:
        printf("wfd status = Find phase started\n");
        break;
    case 5:
        printf("wfd status = Find phase stopped\n");
        break;
    default:
        printf("wfd status = %d\n", data);
        break;
...
/**
 *  @brief Show usage information for the wfd discovery period command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_discovery_period_usage(void)
{
    printf
        ("Usage : wfd_cfg_discovery_period [<MinDiscPeriod> <MaxDiscPeriod>]\n");
    printf
...
/**
 *  @brief Set/get wfd discovery period
 *
 *  Usage: "Usage : wfd_cfg_discovery_period [<MinDiscPeriod> <MaxDiscPeriod>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_discovery_period(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_discovery_period *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_discovery_period_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if ((argc != 2) && (argc != 4)) {
        ...
        return;
               ...
               ((is_input_valid(WFD_MINDISCOVERYINT, 1, &argv[2]) != SUCCESS) ||
                (is_input_valid(WFD_MAXDISCOVERYINT, 1, &argv[3]) !=
                 SUCCESS))) {
        print_wfd_cfg_discovery_period_usage();
        return;
    }

    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_discovery_period);
    ...
    tlv = (tlvbuf_wfd_discovery_period *) (buffer + sizeof(wfd_params_config));

    cmd_buf->cmd_code = HostCmd_CMD_WFD_PARAMS_CONFIG;
    cmd_buf->size = cmd_len - BUF_HEADER_SIZE;
    cmd_buf->seq_num = 0;
    cmd_buf->result = 0;

    tlv->tag = MRVL_WFD_DISC_PERIOD_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_discovery_period) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd Intent command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_intent_usage(void)
{
    printf("Usage : wfd_cfg_intent [IntentValue]\n");
    printf
...
/**
 *  @brief Set/get wfd Intent
 *
 *  Usage: "Usage : wfd_cfg_intent [IntentValue]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_intent(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_intent *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    if (argc < 2) {
        ...
        return;
               ...
               (is_input_valid(WFD_INTENT, argc - 2, argv + 2) != SUCCESS)) {
        print_wfd_cfg_intent_usage();
        return;
    }
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_intent);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_intent *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_INTENT_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_intent) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd Listen Channel command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_listen_channel_usage(void)
{
    printf("Usage : wfd_cfg_listen_channel [ListenChannel]\n");
    printf
...
/**
 *  @brief Set/get wfd ListenChannel
 *
 *  Usage: "Usage : wfd_cfg_listen_channel [ListenChannel]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_listen_channel(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_listen_channel *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_listen_channel_usage();
            return;
        }
    }
    argc -= optind;
    ...
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_listen_channel);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_listen_channel *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_LISTEN_CHANNEL_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_listen_channel) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd Operating Channel command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_op_channel_usage(void)
{
    printf("Usage : wfd_cfg_op_channel [OperatingChannel]\n");
    printf
...
/**
 *  @brief Set/get wfd OperatingChannel
 *
 *  Usage: "Usage : wfd_cfg_op_channel [OperatingChannel]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_op_channel(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_operating_channel *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_op_channel_usage();
            return;
        }
    }
    argc -= optind;
    ...
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_operating_channel);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_operating_channel *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_OPERATING_CHANNEL_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_operating_channel) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd Invitation List command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_invitation_list_usage(void)
{
    printf("\nUsage : wfd_cfg_invitation_list [mac_addr]");
    printf
...
/**
 *  @brief Set/get wfd InvitationList
 *
 *  Usage: "Usage : wfd_cfg_invitation_list [mac_addr]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_invitation_list(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_invitation_list *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0, ctr = 1;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_invitation_list_usage();
            return;
        }
    }
    argc -= optind;
    ...
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_invitation_list);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_invitation_list *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_INVITATION_LIST_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_invitation_list) - MRVL_TLV_HEADER_SIZE;

    if (argc == 2) {
        cmd_buf->action = ACTION_GET;
        cmd_len += (WFD_INVITATION_LIST_MAX - 1) * ETH_ALEN;
    } else {
        cmd_buf->action = ACTION_SET;
            ...
            if (argc == 2) {
                ...
                if (cmd_len > 0) {
                    ...
                    while (cmd_len >= ETH_ALEN) {
                        ...
                        cmd_len -= ETH_ALEN;
                        tlv =
                            (tlvbuf_wfd_invitation_list *) (((t_u8 *) (tlv)) +
                                                            ETH_ALEN);
                    }
                    if (ctr == 1)
                        printf(" empty.");
...
/**
 *  @brief Show usage information for the wfd noa config command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_noa_usage(void)
{
    printf
        ("Usage : wfd_cfg_noa <enable|disable> <index> [<counttype> <duration> <interval>]\n");
    printf
...
/**
 *  @brief Set/get wfd NoA parameters
 *
 *  Usage: "Usage : wfd_cfg_noa <enable|disable> <index> [<counttype> <duration> <interval>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_noa(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_noa_config *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    if (argc > 2) {
        if (argc == 4) {
            ...
            if (atoi(argv[3]) > WFD_NOA_DESC_MAX) {
                print_wfd_cfg_noa_usage();
                return;
            }
        } else {
            if (atoi(argv[3]) > WFD_NOA_DESC_MAX ||
                (is_input_valid(WFD_COUNT_TYPE, 1, &argv[4]) != SUCCESS) ||
                (is_input_valid(WFD_DURATION, 1, &argv[5]) != SUCCESS) ||
                (is_input_valid(WFD_INTERVAL, 1, &argv[6]) != SUCCESS)) {
                print_wfd_cfg_noa_usage();
                return;
            }
        }
    }
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_noa_config);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_noa_config *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_NOA_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_noa_config) - MRVL_TLV_HEADER_SIZE;

    if (argc == 2) {
        cmd_buf->action = ACTION_GET;
        cmd_len += (sizeof(tlvbuf_wfd_noa_config) - MRVL_TLV_HEADER_SIZE)
            * (WFD_NOA_DESC_MAX - 1);
    } else {
        cmd_buf->action = ACTION_SET;
    ...
    if (ret == SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            if (argc == 2) {
                ...
                while (tlv->length >
                       sizeof(tlvbuf_wfd_noa_config) - MRVL_TLV_HEADER_SIZE) {
...
/**
 *  @brief Show usage information for the wfd opp_ps config command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_opp_ps_usage(void)
{
    printf("Usage : wfd_cfg_opp_ps <enable|disable> [<CTWindow>]\n");
...
/**
 *  @brief Set/get wfd Opportunistic Power Save
 *
 *  Usage: "Usage : wfd_cfg_opp_ps <enable|disable> [<CTWindow>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_opp_ps(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_opp_ps_config *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    ...
    if (argc == 4) {
        ...
        if (is_input_valid(WFD_CTWINDOW, 1, &argv[3]) != SUCCESS) {
            print_wfd_cfg_opp_ps_usage();
            return;
        }
    }

    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_opp_ps_config);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_opp_ps_config *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_OPP_PS_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_opp_ps_config) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd capability config command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_capability_usage(void)
{
    printf("Usage : wfd_capability [<DeviceCapability> <GroupCapability>]\n");
    printf
...
/**
 *  @brief Set/get wfd Capability
 *
 *  Usage: "Usage : wfd_cfg_capability [<DeviceCapability> <GroupCapability>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_capability(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_capability_config *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_capability_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if (argc == 4) {
        if ((is_input_valid(WFD_DEVICECAPABILITY, 1, &argv[2]) != SUCCESS) ||
            (is_input_valid(WFD_GROUPCAPABILITY, 1, &argv[3]) != SUCCESS)) {
            print_wfd_cfg_capability_usage();
            return;
        }
    }
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_capability_config);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_capability_config *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_CAPABILITY_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_capability_config) - MRVL_TLV_HEADER_SIZE;
...
/**
 *  @brief Show usage information for the wfd_cfg_persistent_group_record command
 *
 *  $return         N/A
 */
static void
print_wfd_peristent_group_usage(void)
{
    printf("\nUsage : wfd_cfg_persistent_group_record [index] \
...
/**
 *  @brief Creates a wfd_persistent group record request and
 *         sends to the driver
 *
 *  Usage: "Usage : wfd_cfg_cmd_persistent_group_record [index] <
 *                  <role> <bssid> <groupdevid> <ssid> <psk>>
 *                  [<peermac1> <peermac2> ]"
 *
 *  @param argc     Number of arguments
 ...
 **/
static void
wfd_cfg_cmd_persistent_group_record(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_peristent_group_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if (argc > 2) {
        index = atoi(argv[2]);
        if (index >= WFD_PERSISTENT_GROUP_MAX) {
    ...
    default:
        ...
        if (!strncasecmp("0x", argv[7], 2)) {
            argv[7] += 2;
            if (strlen(argv[7]) / 2 != WFD_PSK_LEN_MAX) {
                printf("ERR:Incorrect PSK length %d. It should be %d.\n",
                       strlen(argv[7]) / 2, WFD_PSK_LEN_MAX);
                goto done;
            }
            string2raw(argv[7], tlv->psk + var_len);
            *(&tlv->psk_len + var_len) = WFD_PSK_LEN_MAX;
            var_len += WFD_PSK_LEN_MAX;
        } else {
            /* ascii */
            psk_len = strlen(argv[7]);
            if (psk_len < WFD_PASSPHRASE_LEN_MIN || psk_len >= WFD_PSK_LEN_MAX) {
                printf
        ...
        tlv->length = sizeof(tlvbuf_wfd_persistent_group) - MRVL_TLV_HEADER_SIZE
            + var_len;
    ...
    /* Send collective command */
    wfd_ioctl((t_u8 *) buffer, &cmd_len, buf_len);
    ...
    if (argc == 2 || argc == 3) {
        ...
        buf_len = tlv->length;
        while (buf_len > (sizeof(tlvbuf_wfd_persistent_group) - 2 * ETH_ALEN)) {
            printf("\n\t Index = [%d]\n", tlv->rec_index);
            if (tlv->dev_role == 1)
            ...
            if (tlv->dev_role == 1) {
                ...
                tlv = (tlvbuf_wfd_persistent_group *) (((t_u8 *) (tlv)) +
                                                       (sizeof
...
/**
 *  @brief Show usage information for the wfd_cfg_persistent_group_invoke command
 *
 *  $return         N/A
 */
static void
print_wfd_peristent_group_invoke_usage(void)
{
    printf("\nUsage : wfd_cfg_persistent_group_invoke [index] | <cancel>\n");
    return;
}

/**
 *  @brief Creates a wfd_persistent group Invoke request and
 *         sends to the driver
 *
 *  Usage: "Usage : wfd_cfg_cmd_persistent_group_invoke [index] | <cancel>"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         SUCCESS or FAILURE
 **/
static void
wfd_cfg_cmd_persistent_group_invoke(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_persistent_group *tlv = NULL;
    t_u8 index;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_peristent_group_invoke_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if (!isdigit(argv[2][0])) {
        if (strcmp(argv[2], "cancel")) {
            ...
            index = WFD_PERSISTENT_RECORD_CANCEL;
    }
    tlv->rec_index = index;
    ...
    printf("Persistent group %s successful!\n",
           (index == WFD_PERSISTENT_RECORD_CANCEL) ? "cancel" : "invoke");
  done:
    if (buffer)
        free(buffer);
    return;
...
/**
 *  @brief Show usage information for the wfd presence request parameters command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_presence_req_params_usage(void)
{
    printf
        ("Usage : wfd_cfg_presence_req_params [<type> <duration> <interval>]\n");
...
/**
 *  @brief Set/get wfd presence request parameters
 *
 *  Usage: "Usage : wfd_cfg_presence_req_params [<type> <duration> <interval>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_presence_req_params(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_presence_req_params *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_presence_req_params_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if (argc < 2) {
        ...
        return;
                              ...
                              ||
                              (is_input_valid
                               (WFD_PRESENCE_REQ_TYPE, 1, &argv[2]) != SUCCESS)
                              || (is_input_valid(WFD_DURATION, 1, &argv[3]) !=
                                  SUCCESS)
                              || (is_input_valid(WFD_INTERVAL, 1, &argv[4]) !=
                                  SUCCESS))) {
        print_wfd_cfg_presence_req_params_usage();
        return;
    }
    cmd_len =
        sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_presence_req_params);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv =
        (tlvbuf_wfd_presence_req_params *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->tag = MRVL_WFD_PRESENCE_REQ_PARAMS_TLV_ID;
    tlv->length = sizeof(tlvbuf_wfd_presence_req_params) - MRVL_TLV_HEADER_SIZE;
    ...
    if (ret == SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            if (argc == 2) {
                ...
                printf("Duration of NoA descriptor = %ld\n",
                       le32_to_cpu(tlv->duration));
                printf("Interval of NoA descriptor = %ld\n",
                       le32_to_cpu(tlv->interval));
            } else {
...
/**
 *  @brief Show usage information for the wfd extended listen timing parameters command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_ext_listen_time_usage(void)
{
    printf("Usage : wfd_cfg_ext_listen_time [<duration> <interval>]\n");
...
/**
 *  @brief Set/get wfd extended listen time parameters
 *
 *  Usage: "Usage : wfd_cfg_ext_listen_time [<duration> <interval>]"
 *
 *  @param argc     Number of arguments
 *  @param argv     Pointer to the arguments
 *  @return         None
 */
static void
wfd_cfg_cmd_ext_listen_time(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_mrvl_ext_listen_time *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_ext_listen_time_usage();
            return;
        }
    }
    argc -= optind;
    ...
    if (argc < 2) {
        ...
        return;
                              ...
                              ||
                              (is_input_valid
                               (WFD_ATTR_EXTENDED_TIME, 1, &argv[2]) != SUCCESS)
                              ||
                              (is_input_valid
                               (WFD_ATTR_EXTENDED_TIME, 1,
                                &argv[3]) != SUCCESS))) {
        print_wfd_cfg_ext_listen_time_usage();
        return;
    }
    cmd_len =
        sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_mrvl_ext_listen_time);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv =
        (tlvbuf_wfd_mrvl_ext_listen_time *) (buffer +
    ...
    tlv->tag = MRVL_WFD_EXTENDED_LISTEN_TIME_TLV_ID;
    tlv->length =
        sizeof(tlvbuf_wfd_mrvl_ext_listen_time) - MRVL_TLV_HEADER_SIZE;
    if (argc == 2) {
        cmd_buf->action = ACTION_GET;
    } else {
        cmd_buf->action = ACTION_SET;
...
/**
 *  @brief Show usage information for the wfd provisioing parameters command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_provisioning_params_usage(void)
{
    printf
        ("Usage : wfd_cfg_provisioning_params [<action> <configMethod> <devPassword>]\n");
...
/**
 *  @brief Set/get wfd provisioning protocol related parameters
 *
 ...
 */
static void
wfd_cfg_cmd_provisioning_params(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_provisioning_params *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    int opt, ret = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_provisioning_params_usage();
            return;
        }
    }
    argc -= optind;
    ...
    cmd_len =
        sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_provisioning_params);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv =
        (tlvbuf_wfd_provisioning_params *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->length = sizeof(tlvbuf_wfd_provisioning_params) - MRVL_TLV_HEADER_SIZE;
    if (argc == 2) {
        cmd_buf->action = ACTION_GET;
    } else {
        cmd_buf->action = ACTION_SET;
...
/**
 *  @brief Show usage information for the wfd WPS params command
 *
 *  @return         N/A
 */
static void
print_wfd_cfg_wps_params_usage(void)
{
    printf("\nUsage : wfd_cfg_wps_params [pin/pbc/none]");
...
/**
 *  @brief Set/get wfd WPS parametters
 *
 ...
 */
static void
wfd_cfg_cmd_wps_params(int argc, char *argv[])
{
    wfd_params_config *cmd_buf = NULL;
    tlvbuf_wfd_wps_params *tlv = NULL;
    t_u8 *buffer = NULL;
    t_u16 cmd_len = 0;
    ...
    while ((opt = getopt_long(argc, argv, "+", cmd_options, NULL)) != -1) {
        switch (opt) {
        default:
            print_wfd_cfg_wps_params_usage();
            return;
        }
    }
    argc -= optind;
    ...
    cmd_len = sizeof(wfd_params_config) + sizeof(tlvbuf_wfd_wps_params);
    ...
    cmd_buf = (wfd_params_config *) buffer;
    tlv = (tlvbuf_wfd_wps_params *) (buffer + sizeof(wfd_params_config));
    ...
    tlv->length = sizeof(tlvbuf_wfd_wps_params) - MRVL_TLV_HEADER_SIZE;
...
is_input_valid(valid_inputs cmd, int argc, char *argv[])
{
    ...
    case WFD_DISC_SERVICEPROTO:
        ...
        break;
    case WFD_SERVICEUPDATE_INDICATOR:
        if (argc != 1) {
            printf
        ...
        break;
    case WFD_DISC_SERVICETRANSACID:
        if (argc != 1) {
            printf
...
/** WFD command table */
static command_table wfd_command[] = {
    {"wfd_config", wfdcmd_config, "\tSet/get wfd configuration"},
    {"wfd_params_config", wfdcmd_params_config,
     "\tSet/get wfd parameter's configuration"},
    {"wfd_action_frame", wfdcmd_action_frame, "\tSend wfd action frame."},
    {"wfd_mode", wfdcmd_status, "\tSet/get WFD start/stop status"},
    {"wfd_discovery_request", wfdcmd_service_discovery,
     "Send wfd service discovery request"},
    {"wfd_discovery_response", wfdcmd_service_discovery,
     "Send wfd service discovery response"},
    {"wfd_gas_comeback_request", wfdcmd_gas_comeback_discovery,
     "Send wfd GAS comeback request frame"},
    {"wfd_gas_comeback_response", wfdcmd_gas_comeback_discovery,
     "Send wfd GAS comeback response frame"},
    {"wfd_cfg_persistent_group_record", wfd_cfg_cmd_persistent_group_record,
     "Set/get information about a persistent group"},
    {"wfd_cfg_persistent_group_invoke", wfd_cfg_cmd_persistent_group_invoke,
     "Invoke or disable a persistent group"},
    {"wfd_cfg_discovery_period", wfd_cfg_cmd_discovery_period,
     "\tSet/get discovery period"},
    {"wfd_cfg_intent", wfd_cfg_cmd_intent, "\tSet/get intent"},
    {"wfd_cfg_capability", wfd_cfg_cmd_capability, "\tSet/get capability"},
    {"wfd_cfg_noa", wfd_cfg_cmd_noa, "\tSet/get notice of absence"},
    {"wfd_cfg_opp_ps", wfd_cfg_cmd_opp_ps, "\tSet/get Opportunistic PS"},
    {"wfd_cfg_invitation_list", wfd_cfg_cmd_invitation_list,
     "\tSet/get invitation list"},
    {"wfd_cfg_listen_channel", wfd_cfg_cmd_listen_channel,
     "\tSet/get listen channel"},
    {"wfd_cfg_op_channel", wfd_cfg_cmd_op_channel,
     "\tSet/get operating channel"},
    {"wfd_cfg_presence_req_params", wfd_cfg_cmd_presence_req_params,
     "\tSet/get presence request parameters"},
    {"wfd_cfg_ext_listen_time", wfd_cfg_cmd_ext_listen_time,
     "Set/get extended listen timing parameters"},
    {"wfd_cfg_provisioning_params", wfd_cfg_cmd_provisioning_params,
     "Set/get provisioning protocol related parameters"},
    {"wfd_cfg_wps_params", wfd_cfg_cmd_wps_params,
     "Set/get WPS protocol related parameters"},
    {NULL, NULL, 0}
};

/**
 *  @brief Entry function for wfdutl
 *  @param argc         number of arguments
...
main(int argc, char *argv[])
{
    ...
    /* Process command */
    for (i = 0; wfd_command[i].cmd; i++) {
        if (strncmp(wfd_command[i].cmd, argv[2], strlen(wfd_command[i].cmd)))
            continue;
        if (strlen(wfd_command[i].cmd) != strlen(argv[2]))
            continue;
        wfd_command[i].func(argc, argv);
        break;
    }
    if (!wfd_command[i].cmd) {
