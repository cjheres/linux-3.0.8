...
woal_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    for (i = 0; woal_usb_table[i].idVendor; i++) {
        if (udev->descriptor.idVendor == woal_usb_table[i].idVendor &&
            ...
            if (udev->descriptor.idProduct == USB8797_PID_1)
                usb_cardp->boot_state = USB8797_FW_DNLD;
            else if (udev->descriptor.idProduct == USB8797_PID_2)
                usb_cardp->boot_state = USB8797_FW_READY;
            break;
        }
    }
