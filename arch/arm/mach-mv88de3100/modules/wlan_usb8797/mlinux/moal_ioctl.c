...
/**
 *  @brief management fram tx ioctl handler
 *
 *  @param dev      A pointer to net_device structure
 *  @param req      A pointer to ifreq structure
 *  @return         0 --success, otherwise fail
 */
int
woal_mgmt_frame_tx(struct net_device *dev, struct ifreq *req)
{
    moal_private *priv = (moal_private *) netdev_priv(dev);
    ...
    /* Sanity check */
    if (req->ifr_data == NULL) {
        PRINTM(MERROR, "woal_mgmt_frame_tx() corrupt data\n");
        ret = -EFAULT;
        goto done;
    }
    ...
    /* get whole packet now */
    if (copy_from_user(pmbuf->pbuf + pmbuf->data_offset, req->ifr_data,
                       packet_len + sizeof(packet_len))) {
        PRINTM(MERROR, "Copy from user failed\n");
        ret = -EFAULT;
    ...
    pmbuf->data_len = packet_len + sizeof(packet_len);
 ...
 *
 *  @param priv         A pointer to moal_private structure
 *  @param priv         Action: set or get
 *  @param mgmt_subtype_mask    A Pointer to mgmt frame subtype mask
 *  @param wait_option  wait option (MOAL_WAIT or MOAL_NO_WAIT)
 *
 *  @return             0 --success, otherwise fail
 */
