...
/** Driver version */
char driver_version[] =
    "USB8797-%s-M2614" MLAN_RELEASE_VERSION "-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
    " ";
...
woal_add_card_dpc(moal_handle * handle)
{
    ...
    if (handle->boot_state == USB8797_FW_DNLD) {
        /* Return now */
        LEAVE();
        return ret;
    }
...
woal_init_fw_dpc(moal_handle * handle)
{
    ...
    if (handle->firmware) {
        ...
        if (handle->boot_state == USB8797_FW_DNLD) {
            ...
            if (udev) {
#define USB8797_WAIT_FW_READY  (500)
                mdelay(USB8797_WAIT_FW_READY);
...
woal_init_fw(moal_handle * handle)
{
    ...
    if (MTRUE && (handle->skip_fw_dnld != MTRUE)
        && (handle->boot_state != USB8797_FW_READY)
        ) {
