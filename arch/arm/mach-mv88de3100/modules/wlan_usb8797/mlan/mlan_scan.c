...
#ifdef WIFI_DIRECT_SUPPORT
    /* continue to judge if there is any more P2P IE */
    if (pmpriv->wps.wps_ie.vend_hdr.len < IEEE_MAX_IE_SIZE - 2) {
        ie = (IEEEtypes_VendorSpecific_t *) (pmpriv->wps.wps_ie.vend_hdr.oui
                                             + pmpriv->wps.wps_ie.vend_hdr.len);
        if (ie->vend_hdr.element_id == VENDOR_SPECIFIC_221
            && ie->vend_hdr.len > 0) {
            tlv = (MrvlIEtypesHeader_t *) * pptlv_out;
            tlv->type = wlan_cpu_to_le16(VENDOR_SPECIFIC_221);
            tlv->len = wlan_cpu_to_le16(ie->vend_hdr.len);
            *pptlv_out += sizeof(MrvlIEtypesHeader_t);
            memcpy(pmpriv->adapter, *pptlv_out,
                   ie->vend_hdr.oui, ie->vend_hdr.len);
            /* this needs to be verified */
            *pptlv_out += ie->vend_hdr.len;
        }
    }
#endif
    ...
    *pscan_current_only = MFALSE;
    ...
    if (puser_scan_in) {
        ...
        for (ssid_idx = 0; ((ssid_idx < NELEMENTS(puser_scan_in->ssid_list))
            ...
            /* max_ssid_length = 0 tells firmware to perform specific scan for
               the SSID filled */
            pwildcard_ssid_tlv->max_ssid_length = 0;
