...
#define MV88DE3100_WIFI_WAKEUP_GPIO 4
#define MV88DE3100_BT_WAKEUP_GPIO 4

#ifndef MV88DE3100_WIFI_WAKEUP_GPIO
...
woal_enable_hs(moal_private * priv)
{
    ...
    hscfg.conditions = 0;
    hscfg.gap = 100;
    hscfg.gpio = MV88DE3100_WIFI_WAKEUP_GPIO;
    if (woal_set_get_hs_params(priv, MLAN_ACT_SET, MOAL_IOCTL_WAIT, &hscfg) ==
        MLAN_STATUS_FAILURE) {
        printk("IOCTL request HS enable failed\n");
        goto done;
    }
