...
#define gcdkUSE_NON_PAGED_MEMORY_CACHE 32
    ...
    )
{
    ...
    else if (Handle != gcvNULL && ((PLINUX_MDL)Handle)->contiguous)
    {
        /* Contigous virtual memory. */
        map = FindMdlMap((PLINUX_MDL)Handle, (gctINT) ProcessID);

        gcmkASSERT(map != gcvNULL);

        paddr = virt_to_phys(map->vmaAddr);
    ...
    else
    {
        ...
        {
            paddr = page_to_phys(vmalloc_to_page(map->vmaAddr + PAGE_SIZE * i));
