...
#ifndef GPIO_IOCTL_LOCK
#define GPIO_IOCTL_LOCK       0x1115
#endif

#ifndef GPIO_IOCTL_UNLOCK
#define GPIO_IOCTL_UNLOCK     0x1116
#endif

#ifndef SM_GPIO_IOCTL_LOCK
#define SM_GPIO_IOCTL_LOCK    0x3337
#endif

#ifndef SM_GPIO_IOCTL_UNLOCK
...
static inline void gpio_lock(void)
{
    ...
    if (gpio_fd >= 0)
        ioctl(gpio_fd, GPIO_IOCTL_LOCK, 0);
    else
        pthread_mutex_lock(&gpio_mutex);
}
...
{
    if (gpio_fd >= 0)
        ioctl(gpio_fd, GPIO_IOCTL_UNLOCK, 0);
    else
        pthread_mutex_unlock(&gpio_mutex);
}
...
static inline void sm_gpio_lock(void)
{
    ...
    if (gpio_fd >= 0)
        ioctl(gpio_fd, SM_GPIO_IOCTL_LOCK, 0);
    else
        pthread_mutex_lock(&gpio_mutex);
}
...
{
    if (gpio_fd >= 0)
        ioctl(gpio_fd, SM_GPIO_IOCTL_UNLOCK, 0);
    else
        pthread_mutex_unlock(&gpio_mutex);
}
#endif
