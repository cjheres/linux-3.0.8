...
static DECLARE_WAIT_QUEUE_HEAD(i2c_writeread_wait);
#endif
...
#if defined(FPGA)
#define I2C_SYS_CLOCK   5   // 5 MHz
#define I2C_SM_CLOCK   5     // 5 MHz
#else
#if (BERLIN_CHIP_VERSION >= BERLIN_B_0)
#define I2C_SYS_CLOCK   75     // 75 MHz
#define I2C_SM_CLOCK   20     // 20 MHz
#else
#define I2C_SYS_CLOCK   65     // 65 MHz
#endif
#endif
...
#define I2C_RegRead(a, pv)  GA_REG_WORD32_READ(base_addr+(a), pv) /*(*(volatile int *)(base_addr+(a)))*/
#define I2C_RegWrite(a, v)  GA_REG_WORD32_WRITE(base_addr+(a), v) /**(volatile int *)(base_addr+(a)) = (v)*/
...
const unsigned int I2C_MASTER_BASEADDR[I2C_MASTER_NUM] = {APB_I2C_INST0_BASE, APB_I2C_INST1_BASE, APB_I2C_INST2_BASE, APB_I2C_INST3_BASE};
...
int i2c_master_writeread_bytes(int master_id, int slave_addr, int addr_type, unsigned char *pwbuf, int wnum, unsigned char *prbuf, int rnum, int dum )
{
   unsigned int base_addr;
   int i;
#ifdef POLLING_MODE // polling mode
        int j, k;
#endif
...
#ifndef POLLING_MODE // polling mode
   ...
   if (wait_event_interruptible(i2c_writeread_wait, transaction_done[master_id])) {
    I2C_RegWrite(I2C_REG_ENABLE, ~EN_ENABLE);
    return -ERESTARTSYS;
   }
#else
   // wait until transaction finishes
   while (!transaction_done[master_id]);
#endif
...
if (istat & INTR_TX_ABRT){ // TX ABORT (fail)
   ...
   else if (istat & INTR_TX_EMPTY){ // TX fifo equal or below threshold
    ...
    // copy bytes to TX fifo
    for (i=0; i<MIN(I2C_TX_FIFO_SIZE[master_id]-bytes_in_fifo, bytes_to_be_written); i++){
        ...
        else //if (bytes_written_to_txfifo[master_id]+i < bytes_of_write_cmd[master_id]+bytes_of_read_cmd[master_id])
            I2C_RegWrite(I2C_REG_DATA_CMD, READ_CMD);
    }
    // this is the totally bytes of commands which have been written to TX fifo so far
    bytes_written_to_txfifo[master_id] += i;

    // check if the TX fifo can handle all the remaining bytes
    if (bytes_written_to_txfifo[master_id]==bytes_of_write_cmd[master_id]+bytes_of_read_cmd[master_id]){
        // set TX fifo threshold to 0 to trigger the last TXE interrupt
        set_txfifo_threshold(master_id, 0);
    }

    return;

   } else if (istat & INTR_RX_FULL){ // RX fifo equal or above threshold

    // number of valid bytes t in RX fifo
    I2C_RegRead(I2C_REG_RXFLR, &bytes_in_fifo);
    // copy bytes from TX fifo to buffer
    //for (i=0; i<MIN(bytes_in_fifo, 256-bytes_read_from_rxfifo[master_id]); i++){
