...
MV_CC_ServiceID_U32_t MV_CC_DSS_GetDynamicSID(pMV_CC_DSP_t 		self)
{
	...
	if (!(self->m_SeqID & MV_CC_SID_BIT_DYNAMIC))
	    MV_CC_DBG_Warning(E_OUTOFRANGE, "MV_CC_DSS_GetDynamicSID rolled !!!", NULL);

	MV_OSAL_Mutex_Unlock(self->m_SeqIDMutex);

	return (self->m_SeqID | MV_CC_SID_BIT_DYNAMIC | MV_CC_SID_BIT_NORMAL | MV_CC_SID_BIT_LOCAL);
}
...
HRESULT MV_CC_DSS_Init(void)
{
	...
	if (pMV_APP_DSS == NULL)
	    MV_CC_DBG_Error(E_OUTOFMEMORY, "MV_CC_DSS_Init MV_OSAL_Malloc", NULL);
	...
	if (res != S_OK)
	    MV_CC_DBG_Error(res, "MV_CC_DSS_Init MV_CC_DSS_GlobalServiceList_Init", NULL);
...
HRESULT MV_CC_DSS_Exit(void)
{
	...
	if (res != S_OK)
	    MV_CC_DBG_Error(res, "MV_CC_DSS_Exit MV_CC_DSS_GlobalServiceList_Exit", NULL);
...
HRESULT singlenode_exit(MV_CC_Node *head)
{
	...
	if(head->pnext == NULL) {
		...
		printk("singlenode_exit fail, the singlenode is not empty\n");
	}
	return 0;
}
...
HRESULT MV_CC_DSS_Reg(pMV_CC_DSS_ServiceInfo_t pSrvInfo,MV_CC_Task *cc_task)
{
	...
	//printk("\n\nsinglenode_add ServiceID[%08x] taskid[%d]\n",pSrvInfo->m_ServiceID,cc_task->cc_taskid);
	//singlenode_display(cc_task->serverid_head);
	// Only for test
...
HRESULT MV_CC_DSS_Update(pMV_CC_DSS_ServiceInfo_t pSrvInfo)
{
	...
	if (pSrvInfo_Search != NULL)
		GaloisMemcpy(pSrvInfo_Search, pSrvInfo, sizeof(MV_CC_DSS_ServiceInfo_t));
	else
	{
	    pMV_APP_DSS->m_Status.m_UpdateErrCount++;
	    goto MV_CC_DSS_Update_Failure;
...
HRESULT MV_CC_DSS_Free(pMV_CC_DSS_ServiceInfo_t pSrvInfo,MV_CC_Task *cc_task)
{
	...
	//printk("singlenode_delete ServiceID[%08x] taskid[%d]\n", pSrvInfo->m_ServiceID,cc_task->cc_taskid);
	//singlenode_display(cc_task->serverid_head);
...
HRESULT MV_CC_DSS_GetStatus(pMV_CC_DSS_Status_t pStatus)
{
    ...
    GaloisMemcpy(pStatus, &(pMV_APP_DSS->m_Status), sizeof(MV_CC_DSS_Status_t));
